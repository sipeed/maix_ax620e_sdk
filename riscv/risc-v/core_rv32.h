...
/* Memory mapping of E907 Hardware */
...
static void u32_add_u32_ret(uint32_t a, uint32_t b, str_uint64_t *ret)
{
    ret->low = a + b;
    ret->high = (ret->low < a) ? 1 : 0;
}
static void u64_add_u64_ret(str_uint64_t *a, str_uint64_t *b)
{
    uint32_t temp;
    temp = a->high;
    u32_add_u32_ret(a->low, b->low, a);
    a->high += (temp + b->high);
}
...
{
    uint64_t tick_temp = ticks, tick_ret;
    if ((CORET->MTIMECMP != 0) && (CORET->MTIMECMP != 0xffffffffffffffff)) {
        tick_ret = CORET->MTIMECMP;
        u64_add_u64_ret((str_uint64_t *)&tick_ret, (str_uint64_t *)&tick_temp);
        CORET->MTIMECMP = tick_ret;
    } else {
        tick_ret = CORET->MTIME;
        u64_add_u64_ret((str_uint64_t *)&tick_ret, (str_uint64_t *)&tick_temp);
        CORET->MTIMECMP = tick_ret;
    }
    return (0UL);
