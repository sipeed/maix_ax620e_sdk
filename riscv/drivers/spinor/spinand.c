...
static int spinand_read_from_cache_op(u8 *pbuf, u32 column, u32 nbytes)
{
	...
	if (spinand_continuous_read_support && !spinand_bufmode) {
		...
		if (spi_cmd_addr_dummy_cs_bypass == 1) {
			...
			AX_LOG_DGB("spi_cmd_addr_dummy_cs_bypass = %d", spi_cmd_addr_dummy_cs_bypass);
			op.dummy.nbytes = 4;
			op.dummy.buswidth = 1;
		}
		else {
	...
	while (nbytes) {
		...
		if (ret)
		{
			AX_LOG_ERROR("spi_mem_adjust_op_size fail, op.data.nbytes=%d", op.data.nbytes);
			return ret;
		}
		...
		if (ret)
		{
			AX_LOG_ERROR("spi_mem_exec_op fail, ret = %d", ret);
			return ret;
		}
...
static int spinand_read_page(u32 addr, u8 *pbuf, u32 nbytes, u8 oob_op)
{
	...
	AX_LOG_DGB("addr=0x%x, nbytes=0x%x, oob_op=%d, ram=0x%x", addr, nbytes, oob_op, (u32)pbuf);
	if (oob_op && nbytes > (1 << spinand_oob_size_shift))
	{
		AX_LOG_ERROR("oob_op = %d, nbytes = 0x%x, oob_shift=%d", oob_op, nbytes, spinand_oob_size_shift);
		return -1;
	}
#if defined (SPINAND_CONTINUOUS_READ_SUPPORT)
	if (oob_op && !spinand_bufmode) {
		AX_LOG_ERROR("oob_op=%d not support BUF=%d", oob_op, spinand_bufmode);
		return -1;
	}
#endif
	...
	if (ret)
	{
		AX_LOG_ERROR("spinand_load_page_op fail, row = 0x%x", row);
		return ret;
	}
	...
	if (ret < 0)
	{
		AX_LOG_ERROR("spinand_wait fail, status=0x%x", status);
		return ret;
	}
	}
	...
	if (ret)
	{
		AX_LOG_ERROR("spinand_read_from_cache_op fail, column=0x%x, nbytes=0x%x, ram=0x%x", column, nbytes, (u32)pbuf);
		return ret;
	}
...
int spinand_block_isbad(u32 addr)
{
	...
	spinand_eraseblock_no = addr >> spinand_blk_size_shift;
	ret = spinand_read_page(addr, oob_buf, spinand_oob_len, oob_op);
	if (ret)
		return ret;
...
static int spinand_read_blk(u32 addr, u32 nbytes, char *pbuf)
{
	...
	AX_LOG_DGB("addr=0x%x, nbytes=0x%x, ram=0x%x", addr, nbytes, (u32)pbuf);
	spinand_eraseblock_no = addr >> spinand_blk_size_shift;
	...
	for (i = 0; i < nbytes; ) {
		...
		if (ret < 0)
		{
			AX_LOG_ERROR("spinand_read_page fail: offset=0x%x, page_remain=0x%x, oob_op=%d, ram=0x%x",
...
static int spinand_buf_enable(u8 enable)
{
	...
	if (ret) {
		AX_LOG_ERROR("modify bufmode fail, current bufmode = %d, enable = %d", spinand_bufmode, enable);
		return ret;
	}
...
static int spinand_region_bad_block_scan(u32 addr, u32 nbytes, u16 * table, int table_size)
{
	...
	AX_LOG_DGB("from addr 0x%x, size 0x%x, start blk %d, need scan %d valid blks", addr, nbytes, blk_start, valid_blks);

	while (i < valid_blks) {
		if (0 == spinand_block_isbad(phy_off)) {
			i++;
			AX_LOG_DGB("blk %d is good", phy_off >> spinand_blk_size_shift);
		}
		else {
			if (index >= table_size) {
				AX_LOG_ERROR("%d bad block exeed limit %d", index + 1, table_size);
				return -1;
			}
			table[index++] = (phy_off >> spinand_blk_size_shift);
			AX_LOG_DGB("blk %d is bad", phy_off >> spinand_blk_size_shift);
		}
...
int spinand_read(u32 addr, u32 nbytes, char *pbuf)
{
	...
	if (spinand_continuous_read_support) {
		if (!(addr & ((1 << spinand_page_size_shift) - 1)) && (nbytes > (1 << spinand_page_size_shift))) {
			...
			if (bad_blks < 0)
			{
				AX_LOG_ERROR("scan too many bad blocks!");
				return -1;
			}

			while (j < bad_blks) {
				AX_LOG_DGB("scan blk %d is bad, part total %d bad blocks", spinand_bad_info[j], bad_blks);
				j++;
			}
#endif
		...
		if (spinand_continuous_read_support && !spinand_bufmode) {
			for (j = 0; j < bad_blks; j++) {
				if (spinand_bad_info[j] == (phy_off >> spinand_blk_size_shift)) {
					AX_LOG_DGB("nand addr 0x%x, blk %d is bad", phy_off, phy_off / (1 << spinand_blk_size_shift));
					break;
				}
			}
...
#endif
			AX_LOG_ERROR("nand addr 0x%x, blk %d is bad", phy_off, phy_off / (1 << spinand_blk_size_shift));
			phy_off += (1 << spinand_blk_size_shift);
			continue;
		}
#endif
...
#endif
			AX_LOG_ERROR("spinand_read_blk fail: phy_off=0x%x, blk_remain=0x%x, ram=0x%x", phy_off, blk_remain, (u32)(pbuf + i));
			return ret;
		}
