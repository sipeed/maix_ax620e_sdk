...
static SNSSC200AI_OBJ_T sns_sc200aiparams[AX_VIN_MAX_PIPE_NUM];
...
static AX_F32 sc200ai_again2value(float gain, AX_U8 *again_in, AX_U8 *again_de)
{
    AX_U32 i;
    AX_U32 count;
    ...
    for (i = 0; i < count; i++) {
        if (gain > sc200ai_again_table[i].gain) {
            continue;
        } else {
            if (again_in)
            ...
            return sc200ai_again_table[i].gain;
        }
    }
...
static AX_F32 sc200ai_dgain2value(float gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_U8 *dgain_de2)
{
    AX_U32 i;
    AX_U32 count;
    ...
    for (i = 0; i < count; i++) {
        if (gain > sc200ai_dgain_table[i].gain) {
            continue;
        } else {
            ...
            *dgain_de = sc200ai_dgain_table[i].gain_de;
            ...
            return sc200ai_dgain_table[i].gain;
        }
    }
...
static AX_S32 sc200ai_get_gain_table(AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_U32 i;
    AX_S32 ret = 0;
    if (!params)
    ...
    for (i = 0; i < params->nAgainTableSize ; i++) {
        nAgainTable[i] = sc200ai_again_table[i].gain;
        params->pAgainTable = nAgainTable;
    }

    for (i = 0; i < params->nDgainTableSize ; i++) {
        nDgainTable[i] = sc200ai_dgain_table[i].gain;
...
static AX_S32 sc200ai_set_exp_limit(ISP_PIPE_ID nPipeId, AX_F32 fHdrRatio)
{
    ...
    if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = (2 * sns_sc200aiparams[nPipeId].vts - 8) / 2.0;
        ...
        vts_s = (sns_sc200aiparams[nPipeId].vts + 5 * fHdrRatio - 5) / (fHdrRatio + 1);
        sc200ai_set_vts_s(nPipeId, vts_s);
        SNS_DBG("pipe:%d, user_ratio:%f, user_vts:0x%x, vts_s:0x%x\n", nPipeId, fHdrRatio, sns_sc200aiparams[nPipeId].vts, vts_s);

        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = (2 * sns_sc200aiparams[nPipeId].vts - 2 * vts_s - 10) / 2.0;
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX] = 1 / 2.0;
...
AX_S32 sc200ai_cfg_aec_param(ISP_PIPE_ID nPipeId)
{
    ...
    sns_sc200aiparams[nPipeId].vts = sc200ai_get_vts(nPipeId);

    sns_sc200aiparams[nPipeId].line_period = (float)1000000 / sns_obj->sns_mode_obj.fFrameRate / AXSNS_DIV_0_TO_1(sns_sc200aiparams[nPipeId].vts);

    sns_obj->ae_ctrl_param.fTimePerLine = sns_sc200aiparams[nPipeId].line_period;
    ...
    SNS_DBG("inttime min = %f, max = %f, line_period=%f, vts = %d\n",
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX],
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX],
            sns_sc200aiparams[nPipeId].line_period, sns_sc200aiparams[nPipeId].vts);
...
AX_S32 sc200ai_get_sensor_gain_table(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_TABLE_T *params)
{
    ...
    result = sc200ai_get_gain_table(params);
    return result;
...
AX_S32 sc200ai_set_again(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    ...
    gain_value = sc200ai_again2value(nGainFromUser, &Gain_in, &Gain_de);
    if (gain_value == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurAGain[HDR_LONG_FRAME_IDX] = gain_value;
        sc200ai_update_regidx_table(nPipeId, SC200AI_AGAIN_LONG_H_IDX, (Gain_in & 0xFF));
        sc200ai_update_regidx_table(nPipeId, SC200AI_AGAIN_LONG_L_IDX, (Gain_de & 0xFF));
    }
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        if (gain_value == -1) {
            ...
            sc200ai_update_regidx_table(nPipeId, SC200AI_AGAIN_SHORT_H_IDX, (Gain_in & 0xFF));
            sc200ai_update_regidx_table(nPipeId, SC200AI_AGAIN_SHORT_L_IDX, (Gain_de & 0xFF));
            if (result != 0) {
...
AX_S32 sc200ai_set_dgain(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    ...
    gain_val = sc200ai_dgain2value(nGainFromUser, &Gain_in, &Gain_de, &Gain_de2);
    if (gain_val == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
...
static AX_U32 sc200ai_updata_vts(ISP_PIPE_ID nPipeId, AX_U32 vts)
{
    AX_U8 vts_h = 0;
    AX_U8 vts_l = 0;
    AX_S32 result = 0;
    ...
    sc200ai_update_regidx_table(nPipeId, SC200AI_VTS_LONG_H_IDX, vts_h);
    sc200ai_update_regidx_table(nPipeId, SC200AI_VTS_LONG_L_IDX, vts_l);

    return result;
}
#if 0
static AX_U32 sc200ai_updata_vs_vts(ISP_PIPE_ID nPipeId, AX_U32 vts)
{
    AX_U8 vts_h = 0;
    AX_U8 vts_l = 0;
    AX_S32 result = 0;
    ...
    sc200ai_update_regidx_table(nPipeId, SC200AI_VTS_SHORT_H_IDX, vts_h);
    sc200ai_update_regidx_table(nPipeId, SC200AI_VTS_SHORT_L_IDX, vts_l);
...
AX_S32 sc200ai_set_integration_time(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    ...
    SNS_DBG("pipe:%d, userExpline:%f-%f, userRatio:%.2f, curExpline:%f-%f(%f-%f), curRatio:%.2f, line_period:%f\n",
...
AX_S32 sc200ai_set_slow_fps(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    ...
    if (fFps >= framerate) {
        sns_sc200aiparams[nPipeId].vts = vts;
    } else {
        sns_sc200aiparams[nPipeId].vts = 1 * SNS_1_SECOND_UNIT_US / (sns_sc200aiparams[nPipeId].line_period * 2 * fFps);
    }

    if (sns_sc200aiparams[nPipeId].vts > SC200AI_MAX_VTS){
        sns_sc200aiparams[nPipeId].vts = SC200AI_MAX_VTS;
        fFps = 1 * SNS_1_SECOND_UNIT_US / (sns_sc200aiparams[nPipeId].line_period * 2 *sns_sc200aiparams[nPipeId].vts);
        SNS_ERR("Beyond minmum fps  %f\n",fFps);
    }
    result = sc200ai_updata_vts(nPipeId, sns_sc200aiparams[nPipeId].vts);
    sc200ai_set_exp_limit(nPipeId, SC200AI_MAX_RATIO);

    sns_obj->ae_ctrl_param.sns_ae_param.fCurFps = 1 * SNS_1_SECOND_UNIT_US / (sns_sc200aiparams[nPipeId].line_period * 2 *sns_sc200aiparams[nPipeId].vts);

    SNS_DBG("nPipeId = %d, fps(from alg) = %f, current vts = 0x%x\n", nPipeId, fFps, sns_sc200aiparams[nPipeId].vts);
...
AX_U32 sc200ai_sns_refresh_all_regs_from_tbl(ISP_PIPE_ID nPipeId)
{
    ...
    for (i = 0; i < nNum; i++) {
        nRegValue = sc200ai_reg_read(nPipeId, gSc200aiAeRegsTable[i].nRegAddr);
        sns_obj->sztRegsInfo[0].sztI2cData[i].nRegAddr = gSc200aiAeRegsTable[i].nRegAddr;
        sns_obj->sztRegsInfo[0].sztI2cData[i].nData = nRegValue;

        SNS_DBG(" nRegAddr 0x%x, nRegValue 0x%x\n", gSc200aiAeRegsTable[i].nRegAddr, gSc200aiAeRegsTable[i].nRegValue);
    }

    return SNS_SUCCESS;
}

AX_S32 sc200ai_sns_update_init_exposure_reg(ISP_PIPE_ID nPipeId)
{
    int i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
    ...
    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        sc200ai_write_register(nPipeId, sns_obj->sztRegsInfo[0].sztI2cData[i].nRegAddr, sns_obj->sztRegsInfo[0].sztI2cData[i].nData);
        SNS_DBG("Idx = %d, reg addr 0x%x, reg data 0x%x\n",
...
AX_S32 sc200ai_ae_get_sensor_reg_info(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    ...
    if ((AX_FALSE == sns_obj->bSyncInit) || (AX_FALSE == ptSnsRegsInfo->bConfig)) {
        ...
        for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
            ...
            sns_obj->sztRegsInfo[0].sztI2cData[i].nDevAddr = gSc200aiSlaveAddr[nPipeId];
            sns_obj->sztRegsInfo[0].sztI2cData[i].nAddrByteNum = SC200AI_ADDR_BYTE;
            sns_obj->sztRegsInfo[0].sztI2cData[i].nDataByteNum = SC200AI_DATA_BYTE;
            sns_obj->sztRegsInfo[0].sztI2cData[i].nDelayFrmNum = gSc200aiAeRegsTable[i].nDelayFrmNum;
        ...
        // sc200ai_sns_update_init_exposure_reg(nPipeId);
    } else {
        for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
...
AX_S32 sc200ai_get_slow_shutter_param(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
    ...
    if (sns_sc200aiparams[nPipeId].line_period == 0) {
        SNS_ERR("line_period is zero : %f\n", sns_sc200aiparams[nPipeId].line_period);
        return SNS_ERR_CODE_ILLEGAL_PARAMS;
    }
    ...
    ptSlowShutterParam->fMinFps = AXSNS_MAX(gFpsGear[0],
                                            (1 * SNS_1_SECOND_UNIT_US / (sns_sc200aiparams[nPipeId].line_period * 2 * SC200AI_MAX_VTS)));

    for (nfps = 0 ; nfps < ptSlowShutterParam->nGroupNum; nfps++) {
        nVts = 1 * SNS_1_SECOND_UNIT_US / (sns_sc200aiparams[nPipeId].line_period * 2 * gFpsGear[nfps]);
        if ((AX_S32)gFpsGear[nfps] >= framerate) {
            nVts = sns_obj->sns_mode_obj.nVts;
        }
        if (nVts > SC200AI_MAX_VTS) {
            nVts = SC200AI_MAX_VTS;
            SNS_WRN("Beyond minmum fps  %f\n", ptSlowShutterParam->fMinFps);
        }

        if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
            ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime = 2 * nVts - 8;
        } else if (IS_2DOL_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
            ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime = 2 * nVts - 10;
        }

        ptSlowShutterParam->tSlowShutterTbl[nfps].fRealFps = 1 * SNS_1_SECOND_UNIT_US / (sns_sc200aiparams[nPipeId].line_period * 2 * nVts);
        ...
        SNS_DBG("nPipeId = %d, line_period = %.2f, fps = %.2f, fMaxIntTime = %.2f, vts=0x%x\n",
                nPipeId, sns_sc200aiparams[nPipeId].line_period,
    ...
    if (AX_SNS_HDR_2X_MODE == sns_obj->sns_attr_param.eSnsMode) {
        if (IS_SNS_FPS_EQUAL(fFps, 30.0)) {
            ...
            case e_SC200AI_MIPI_24M_792MBPS_2LANE_1920x1088_10BIT_HDR_30FPS:
                sns_sc200aiparams[nPipeId].vts = 0x960;
                break;
            default :
            ...
            case e_SC200AI_MIPI_24M_792MBPS_2LANE_1920x1088_10BIT_HDR_25FPS:
                sns_sc200aiparams[nPipeId].vts = 0xB40;
                break;
            default :
                SNS_ERR("not support hdr mode:%d\n", sns_obj->sns_mode_obj.eHDRMode);
                return SNS_ERR_CODE_ILLEGAL_PARAMS;
        ...
        if (IS_SNS_FPS_EQUAL(fFps, 30.0)) {
            ...
            case e_SC200AI_MIPI_24M_371P25MBPS_2LANE_1920x1088_10BIT_SDR_30FPS:
                sns_sc200aiparams[nPipeId].vts = 0x465;
                break;
            default :
                SNS_ERR("not support hdr mode:%d\n", sns_obj->sns_mode_obj.eHDRMode);
                return SNS_ERR_CODE_ILLEGAL_PARAMS;
            ...
            case e_SC200AI_MIPI_24M_742P5MBPS_1LANE_1920x1080_10BIT_SDR_25FPS:
            case e_SC200AI_MIPI_24M_371P25MBPS_2LANE_1920x1088_10BIT_SDR_30FPS:
            case e_SC200AI_MIPI_24M_371P25MBPS_2LANE_1920x1088_10BIT_SDR_25FPS:
                sns_sc200aiparams[nPipeId].vts = 0x546;
                break;
            default :
                SNS_ERR("not support hdr mode:%d\n", sns_obj->sns_mode_obj.eHDRMode);
                return SNS_ERR_CODE_ILLEGAL_PARAMS;
            }
        }  else if (IS_SNS_FPS_EQUAL(fFps, 15.0)) {
            if (sns_obj->sns_mode_obj.eMasterSlaveSel == AX_SNS_SYNC_SLAVE) {
                sns_sc200aiparams[nPipeId].vts = 0x12bc;
            } else {
                sns_sc200aiparams[nPipeId].vts = 0x12c0;
            }
        }
    }
    for (i = 0; i < AX_VIN_MAX_PIPE_NUM; i++) {
        SENSOR_GET_CTX(i, sns_obj);
        if (sns_obj == AX_NULL) {
            continue;
        }
        if (sns_obj->sns_mode_obj.eMasterSlaveSel == AX_SNS_SYNC_SLAVE) {
            vsync_info.i2c_info[j].i2c_dev = sns_obj->sns_i2c_obj.i2c_dev;
            vsync_info.i2c_info[j].nI2cAddr = gSc200aiSlaveAddr[i];
            vsync_info.i2c_info[j].reg_num = 2;
            vsync_info.i2c_info[j].sns_reg[0].nRegAddr = SC200AI_VTS_LONG_H;
            vsync_info.i2c_info[j].sns_reg[0].nData = (sns_sc200aiparams[nPipeId].vts >> 8) & 0x7f;
            vsync_info.i2c_info[j].sns_reg[1].nRegAddr = SC200AI_VTS_LONG_L;
            vsync_info.i2c_info[j].sns_reg[1].nData = sns_sc200aiparams[nPipeId].vts & 0xff;
            vsync_info.i2c_info[j].nAddrByteNum = SC200AI_ADDR_BYTE;
            vsync_info.i2c_info[j].nDataByteNum = SC200AI_DATA_BYTE;
            vsync_info.i2c_info[j].nDelayFrmNum = 1;
            vsync_info.fps = (AX_U32)fFps;
            j++;
        }
    }
    SENSOR_GET_CTX(nPipeId, sns_obj);
    if (sns_obj->sns_mode_obj.eMasterSlaveSel == AX_SNS_SYNC_SLAVE) {
        ax_mipi_switch_set_fps(&vsync_info);
    } else {
        sc200ai_set_vts(nPipeId, sns_sc200aiparams[nPipeId].vts);
    }
    /* set sensor vts */
    //result = sc200ai_updata_vts(nPipeId, sns_sc200aiparams[nPipeId].vts);
    //sc200ai_set_vts(nPipeId, sns_sc200aiparams[nPipeId].vts);
    // result = sc200ai_updata_vs_vts(nPipeId, (sns_sc200aiparams[nPipeId].vts + 5 * hdr_ratio - 5) / (hdr_ratio + 1));

    /* change fps */
    // sns_obj->sns_attr_param.fFrameRate = fFps;
    sns_obj->sns_mode_obj.nVts = sns_sc200aiparams[nPipeId].vts;
