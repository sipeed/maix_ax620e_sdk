...
#define SNS_LOG_LEVEL_CTRL      (SNS_LOG_WARN)

#define SNS_DBG(format...)  \
do {                            \
    if (SNS_LOG_LEVEL_CTRL <= SNS_LOG_DEBUG) {   \
        rt_kprintf("[%28s][%6d][DEBUG]", __func__, __LINE__);    \
        rt_kprintf(format);\
    }   \
} while(0)
...
do {                            \
    if (SNS_LOG_LEVEL_CTRL <= SNS_LOG_INFO) {    \
        rt_kprintf("[%28s][%6d][INFO ]", __func__, __LINE__);    \
        rt_kprintf(format);\
    }   \
} while(0)
...
do {                            \
    if (SNS_LOG_LEVEL_CTRL <= SNS_LOG_ERROR) {   \
        rt_kprintf("[%28s][%6d][ERROR]", __func__, __LINE__);    \
        rt_kprintf(format);\
    }   \
} while(0)
