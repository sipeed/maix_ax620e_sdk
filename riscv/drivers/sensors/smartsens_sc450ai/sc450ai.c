 ...
 ****************************************************************************/
static AX_S32 sc450ai_ctx_init(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    AX_S32 ret = 0;

    SNS_DBG("sc450ai sc450ai_ctx_init. ret = %d\n", ret);
...
static AX_VOID sc450ai_ctx_exit(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 sc450ai_get_chipid(ISP_PIPE_ID nPipeId, AX_S32 *pSnsId)
{
    ...
    if (sns_obj->sns_id == 0) {
        sensor_id |= sc450ai_reg_read(nPipeId, SC450AI_SENSOR_ID_REG_H) << 8;
        sensor_id |= sc450ai_reg_read(nPipeId, SC450AI_SENSOR_ID_REG_L);

        SNS_DBG("sc450ai id: 0x%x\n", sensor_id);

        if (sensor_id != SC450AI_SENSOR_ID) {
            SNS_ERR("Failed to read sensor sc450ai id:0x%x\n", sensor_id);
...
static void sc450ai_init(ISP_PIPE_ID nPipeId)
{
    ...
    if (AX_NULL == sns_obj) {
        /* contex init */
        nRet = sc450ai_ctx_init(nPipeId);
        if (0 != nRet) {
            SNS_ERR("sc450ai_ctx_init failed!\n");
            return;
        } else {
            SENSOR_GET_CTX(nPipeId, sns_obj);
        }
...
static AX_S32 sc450ai_sensor_set_mode(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *sns_mode)
{
    ...
    {
        sns_setting_index = e_SC450AI_2688X1520_LINEAR_10bit_RGGB_60FPS_4LANE_24M_720Mbps;
        setting_fps = 60;
