...
{
	return ax_readl(gzipdRegBaseVirAddr + addr_offset);
}

static void gzip_reg_write(u32 addr_offset, u32 val)
{
	ax_writel(val, gzipdRegBaseVirAddr + addr_offset);
}
...
int check_gzipd_status(void)
{
	if (gzip_reg_read(GZIPD_STATUS1_REG) & 0x1C) {
		AX_GZIP_LOG_ERR("gzipd work status NOT OK , please check here \n");
		gzipd_dev_show_regvalue();
		return gzipd_error;
	}
	return gzipd_ok;
...
int gzipd_is_bus_busy(void)
{
	u32 loop = 5000;
	u32 mask = 1 << 0 | 1 << 1 | 1 << 2;
	while ((gzip_reg_read(GZIPD_STATUS2_REG) & mask) != 0 && --loop) {
		ax_udelay(100);
	}
	if (loop == 0) {
		AX_GZIP_LOG_ERR("gzipd bus busying alway,reg = 0x%x\n", gzip_reg_read(GZIPD_STATUS2_REG));
		gzipd_dev_show_regvalue();
		return gzipd_error;
	}
	return gzipd_ok;
...
{
	// SW_RST
	ax_writel(GZIPD_SW_RST | GZIPD_CORE_SW_RST, FLASH_SYS_GLB_BASE_ADDR + FLASH_SW_RST0_SET_REG);
	ax_udelay(10);
	ax_writel(GZIPD_SW_RST | GZIPD_CORE_SW_RST, FLASH_SYS_GLB_BASE_ADDR + FLASH_SW_RST0_CLR_REG);

	// select clk
	ax_writel(NPLL_533M << 9 | 0x5 << 6, FLASH_SYS_GLB_BASE_ADDR + FLASH_CLK_MUX0_SET_REG);

	gzipdRegBaseVirAddr = (void *)AX_GZIPD_BASE_PADDR;

#ifdef GZIPD_BYPASS_EN
	ax_writel(NPLL_533M << 9 | 0x5 << 6, FLASH_SYS_GLB_BASE_ADDR + FLASH_CLK_MUX0_REG);
...
int gzipd_dev_get_header_info(void *pData, gzipd_header_info_t *pInfo)
{
	...
	rt_memcpy(&type[0], pData, 2);
	if (rt_memcmp(&type[0], "20", 2)) {
		AX_GZIP_LOG_ERR("This data is without gzipd header info [%x, %x]", *(char *)pData, *((char *)pData + 1));
		return gzipd_error;
	}
	rt_memcpy(pInfo, pData, sizeof(gzipd_header_info_t));
	AX_GZIP_LOG_DBG(" ZIP data info .blk_num = %d, .isize = %d, osize = %d, icrc = %x",
...
int gzipd_dev_cfg(u64 tileSize, void *out_addr, u64 isize, u64 osize, u32 blk_num, u32 *cnt, u32 *last_size)
{
	...
	gzip_reg_write(GZIPD_WDMA_CFG1_REG, osize-1);
	gzip_reg_write(GZIPD_WDMA_CFG0_REG, (u64)(uintptr_t)out_addr >> 3);
	if (tile_cnt == 0) {
...
int gzipd_dev_run(void *tiles_start_addr,  void *tiles_end_addr, u32 *queued_num)
{
	...
	start = (u64)(uintptr_t)tiles_start_addr;
	end = (u64)(uintptr_t)tiles_end_addr;
	AX_GZIP_LOG_DBG("tile data range [0x%8x, 0x%8x]", start, end);
	...
	for (idx = 0; idx < tiles_cnt;) {
		...
		idx++;
		AX_GZIP_LOG_DBG("tile_addr[%3d] = 0x%8x", idx, tile_addr);
	}
	// gzip_reg_write(GZIPD_RDMA_CFG1_REG, tile_addr >> 3);
...
int gzipd_dev_run_last_tile(void *last_tile_start_addr,  u64 last_tile_size)
{
	...
	AX_GZIP_LOG_DBG("last_tile_start_addr =0x%x, last_tile_size = 0x%x", last_tile_start_addr, last_tile_size);
	gzip_reg_write(GZIPD_RDMA_CFG1_REG, (u64)(uintptr_t)last_tile_start_addr >> 3);
	gzip_reg_write(GZIPD_RDMA_CFG2_REG, last_tile_size - 1);
	if (gzipdTriggerFlag == FALSE) {
		gzipdTriggerFlag = TRUE;
		gzip_reg_write(GZIPD_CTRL_CFG_REG,0x1f01u);
