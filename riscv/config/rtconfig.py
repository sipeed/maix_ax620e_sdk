...
if  CROSS_TOOL == 'gcc':
    PLATFORM    = 'gcc'
    EXEC_PATH   = r'/usr/local/riscv64-elf-gnu-amd64/bin'
else:
...
if PLATFORM == 'gcc':
    ...
    CFLAGS  = DEVICE + ' -c -g -ffunction-sections -fdata-sections -Wall -Werror -mcmodel=medlow'
    AFLAGS  = ' -c' + DEVICE + ' -x assembler-with-cpp'
    LFLAGS  = DEVICE + ' -nostartfiles -Wl,--no-whole-archive -T link.lds -lm -lc -lgcc -Wl,-gc-sections -Wl,-zmax-page-size=1024 -Wl,-Map=rtthread.map'
    CPATH   = ''
    LPATH   = ''
