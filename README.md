...
This repository contains the source code for building `MaixCam2`, `KVM Pro`, `NanoKVM-Go`, etc. basic system images.
...
* Build Debian rootfs (NanoKVM-Go / AX620Q)

`qemu-user-static` is used while building the Debian rootfs.
...
This generates `rootfs/arm/glibc/debian/debian_rootfs_base.tar.gz`.
...
## Customize Debian rootfs (NanoKVM-Go / AX620Q)

The Debian base rootfs is generated by `rootfs/arm/glibc/debian/mk_debian_base.sh`. To customize the common Debian base system, modify this script.
...
| Platform   | Rootfs | Project Directory                                            |
| ---------- | ------ | ------------------------------------------------------------- |
...
After building NanoKVM-Go, use the script to generate the SD image:
...
The default output is a complete SD card image under `build/out`, named like:

```text
build/out/AX620Q_emmc_arm32_k419_sipeed_nanoagent_sdcard_<time>.img
```

The image contains an MBR partition table. Write it to the whole SD card device, not to a partition:

```shell
sudo dd if=<sdcard.img> of=/dev/<sd-card-disk> bs=4M status=progress conv=fsync
```

The script looks for the rootfs in `build/out/AX620Q_emmc_arm32_k419_sipeed_nanoagent/objs/debian_rootfs` first.

To generate an initramfs USB recovery update image, run:

```shell
sudo ./gen_sd_image.sh --update
```

This creates a raw ext4 rootfs partition image named like:

```text
build/out/AX620Q_emmc_arm32_k419_sipeed_nanoagent_sd_update_<time>.img
```

Use this image only for the USB Mass Storage device exposed by initramfs as `/dev/mmcblk1p2`. Do not write `sd_update_*.img` to a whole empty SD card, and do not write the full `sdcard_*.img` to the recovery rootfs partition.

### Manual SD card layout
