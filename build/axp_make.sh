...
RISCV_B_PATH=$IMG_PATH/rtthread_b_signed.bin
...
if [[ "$PROJECT" =~ "AX630C_nand" ]] || [[ "$PROJECT" =~ "AX620Q_nand" ]] ||\
	[[ "$PROJECT" =~ "AX620Q_dashcam_nand" ]] ; then
	PARAM_PATH=$IMG_PATH/param.ubi
	ROOTFS_PATH=$IMG_PATH/rootfs_soc_opt.ubi
elif [[ "$PROJECT" =~ "AX620Q_nor" ]] || [[ "$PROJECT" =~ "AX620Q_dashcam_nor" ]] || [[ "$PROJECT" =~ "AX630C_nor" ]]; then
	ROOTFS_PATH=$IMG_PATH/rootfs.img
	OPT_PATH=$IMG_PATH/opt.img
elif [[ "$PROJECT" =~ "AX620Q_fastnor" ]] || [[ "$PROJECT" =~ "AX620Q_minions_fastnor" ]]; then
	ROOTFS_PATH=$IMG_PATH/rootfs.img
elif [[ "$PROJECT" =~ "AX620Q_fastnand_arm32_k419" ]]  || [[ "$PROJECT" =~ "AX630C_fastnand_arm64_k419" ]]; then
	ROOTFS_PATH=$IMG_PATH/rootfs.img
