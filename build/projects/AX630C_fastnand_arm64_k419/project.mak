...
COPY_USB_KO                    := TRUE
COPY_DWMAC_KO                  := TRUE
FDL2_UBOOT_COMPILE_INDEPENDENT := FALSE
COPY_SD_KO                     := TRUE
...
CONFIG_FS_TYPE               := ubifs # SQUASHFS or JFFS2
...
MODEL_PARTITION_SIZE           := 8M
endif
ifeq ($(strip $(AX_FAST_RISCV_RAW_DET_SUPPORT)),TRUE)
RISCV_PARTITION_SIZE           := 4M
else
RISCV_PARTITION_SIZE           := 256K
endif
ROOTFS_PARTITION_SIZE          := 102M
CUSTOMER_PARTITION_SIZE        := 4M
...
OS_MEM_SIZE          := 135 #MB
ISP_IMAGE_LEN_MB     := 65 #MB
# ramdisk size
RAMDISK_SIZE_MB      := 14 #MB
...
FLASH_PARTITIONS  += $(ATF_PART),
endif
FLASH_PARTITIONS  += $(UBOOT_PART),$(ENV_PART),$(CUSTOMER_PART),$(RAWDATA_PART),
...
RAMDISK_BOOTARGS  := "$(OS_MEM) console=ttyS0,115200n8 loglevel=4 earlycon=uart8250,mmio32,0x4880000 board_id=0,boot_reason=0x0,noinitrd \
