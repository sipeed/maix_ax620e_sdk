...
def print_usage():
    ...
    print('-path <path>: eg: ./out/AX630C_emmc_arm64_k419/images\n')
...
def pack_sd_update(path, xmlpath, optee=False, support_ab=False):
    ...
    for file in f:
        try:
            ...
            if(file == 'rtthread_signed.bin'):
                shutil.copyfile(os.path.realpath(file), os.path.dirname(os.path.realpath(file)) + '/sd_update_pack/' + 'riscv.img')
            ...
            if(support_ab):
                ...
                if((file == 'rtthread_signed.bin')):
                    shutil.copyfile(os.path.realpath(file), os.path.dirname(os.path.realpath(file)) + '/sd_update_pack/' + 'riscv_b.img')
                if((file == 'boot_signed.bin')):
