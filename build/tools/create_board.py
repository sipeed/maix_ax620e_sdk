...
def construct_kernel(linux_path, linux_ver, template_board, new_board):
    template_dts = os.path.join(linux_path, linux_ver, f"arch/{ARCH}/boot/dts/axera", f"{template_board}.dts")
    new_dts      = os.path.join(linux_path, linux_ver, f"arch/{ARCH}/boot/dts/axera", f"{new_board}.dts")
    template_defconfig = os.path.join(linux_path, linux_ver, f"arch/{ARCH}/configs", f"axera_{template_board}_defconfig")
    new_defconfig      = os.path.join(linux_path, linux_ver, f"arch/{ARCH}/configs", f"axera_{new_board}_defconfig")
    cmd = f"cp {template_dts} {new_dts}"
    exec_cmd(cmd)
    cmd = f"cp {template_defconfig} {new_defconfig}"
    exec_cmd(cmd)
    makefile = os.path.join(linux_path, linux_ver, f"arch/{ARCH}/boot/dts/axera/Makefile")
