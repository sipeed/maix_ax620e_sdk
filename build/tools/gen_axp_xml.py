...
'''
default_partitions_info = [['init', '0K'], ['eip', '0K'], ['fdl1', '0K'], ['fdl2', '0K'], ['eraseenv', '0K']]
no_block_id = ["init", "eip", "fdl1", "fdl2"]
#default configuration, add XML_IMG_OPTION := "param:select=1", "param:flag=1" in project.mak will overlay this
img_node_configs={
    "param":{"select":0,},
    "rawdata":{"flag":0, "select":0, "type":"ERASEFLASH"},
    "init":{"flag":2, "select":1, "type":"INIT"},
    "eip":{"flag":3, "select":1, "Base":"0x3000000", "type":"EIP"},
    "fdl1":{"flag":3, "select":1, "Base":"0x3000000", "type":"FDL1"},
    "fdl2":{"flag":3, "select":1, "Base":"0x5C000000", "type":"FDL2"},
    "eraseenv":{"flag":0, "select":0, "type":"ERASEFLASH", "block_id":"env"}
}
...
def partitions_info_reorder(prj_name, filename, partitions_info):
    lines = get_project_cfg(prj_name, filename)
    match = re.search(r"'XML_PART_REORDER *:= *(.*?)'", str(lines))
    if match:
        match = match.group(1).replace("'","").replace('"','').replace(" ","").split(',')
        value_to_find = match[0]
        index = None
        for i, sublist in enumerate(partitions_info):
        ...
        append_list=[]
        for i in match:
            for j, sublist in enumerate(partitions_info):
                if i in sublist:
                    append_list.append(sublist)
                    partitions_info.pop(j)
        for i in append_list:
            index += 1
            partitions_info.insert(index, i)
...
def new_img_node(name,size, parent_node):
    ...
    is_no_block_id = False
    block_id = None
    if name in no_block_id:
        is_no_block_id = True
    if is_no_block_id:
        block_node = etree.SubElement(img_node, "Block")
    else:
        block_id = name
        if name in img_node_configs.keys():
            block_id = img_node_configs[name].get("block_id", name)
        block_node = etree.SubElement(img_node, "Block", id=f"{block_id}")
    ...
    size_node.text = "0x0"
    if name in img_node_configs.keys():
        base_node.text = img_node_configs[name].get("Base", "0x0")
        size_node.text = img_node_configs[name].get("Size", "0x0")
...
def gen_new_xml(src_xml, dst_xml, partitions_info):
    ...
    #reorder partitions_info
    partitions_info = default_partitions_info + partitions_info
    partitions_info_reorder(prj_name, project_mak, partitions_info)
