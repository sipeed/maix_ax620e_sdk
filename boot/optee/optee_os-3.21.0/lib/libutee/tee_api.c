...
{
	return utee_efuse_driver_write(blk, data);
}
TEE_Result Tee_Efuse_Read(uint32_t blk, uint32_t *data)
{
	return  utee_efuse_driver_read(blk, data);
}
TEE_Result Tee_Efuse_Init(void)
{
	return utee_efuse_driver_init();
}
TEE_Result Tee_Efuse_Deinit(void)
{
	return utee_efuse_driver_deinit();
}
TEE_Result Tee_Efuse_Blk_Protect(uint32_t blk)
{
	return  utee_efuse_driver_protect(blk);
}
