...
#ifndef __AX_CIPHER_API_H__
#define __AX_CIPHER_API_H__
...
typedef enum {
	...
	AX_CIPHER_INVALID_PARAMETER = 0x80020002,
						      /** Invalid parameter */
	AX_CIPHER_INVALID_KEYSIZE = 0x80020003,
						      /** Invalid key size */
	AX_CIPHER_INVALID_LENGTH = 0x80020004,	      /** Invalid length */
	AX_CIPHER_INVALID_LOCATION = 0x80020005,
						      /** Invalid location */
	...
	AX_CIPHER_DATA_OVERRUN_ERROR = 0x80020009,
						     /** Data overrun error */
	AX_CIPHER_ASSET_CHECKSUM_ERROR = 0x8002000A,
						     /** Asset checksum error */
	AX_CIPHER_INVALID_ASSET = 0x8002000B,	     /** Invalid Asset */
	AX_CIPHER_FULL_ERROR = 0x8002000C,	     /** Full/Overflow error */
	AX_CIPHER_INVALID_ADDRESS = 0x8002000D,
						     /** Invalid address */
	AX_CIPHER_INVALID_MODULUS = 0x8002000E,
						     /** Invalid Modulus */
	AX_CIPHER_VERIFY_ERROR = 0x8002000F,	     /** Verify error */
	AX_CIPHER_INVALID_STATE = 0x80020010,	     /** Invalid state */
	AX_CIPHER_OTP_WRITE_ERROR = 0x80020011,
						     /** OTP write error */
	AX_CIPHER_ASSET_EXPIRED = 0x80020012,	     /** Asset expired error */
	AX_CIPHER_COPROCESSOR_ERROR = 0x80020013,
						     /** Coprocessor error */
	AX_CIPHER_PANIC_ERROR = 0x80020014,	     /** Panic error */
	AX_CIPHER_TRNG_SHUTDOWN_ERROR = 0x80020015,
						     /** Too many FROs shutdown */
	AX_CIPHER_DRBG_STUCK_ERROR = 0x80020016,
						     /** Stuck DRBG */
	AX_CIPHER_UNSUPPORTED = 0x80020017,	    /** Not supported */
	AX_CIPHER_NOT_INITIALIZED = 0x80020018,
						    /** Not initialized yet */
	AX_CIPHER_BAD_ARGUMENT = 0x80020019,	    /** Wrong use; not depending on configuration */
	AX_CIPHER_INVALID_ALGORITHM = 0x8002001A,
						    /** Invalid algorithm code */
	AX_CIPHER_INVALID_MODE = 0x8002001B,	    /** Invalid mode code */
	AX_CIPHER_BUFFER_TOO_SMALL = 0x8002001C,
						    /** Provided buffer too small for intended use */
	AX_CIPHER_NO_MEMORY = 0x8002001D,	    /** No memory */
	AX_CIPHER_OPERATION_FAILED = 0x8002001E,
						    /** Operation failed */
	AX_CIPHER_TIMEOUT_ERROR = 0x8002001F,
						    /** Token or data timeout error */
	AX_CIPHER_INTERNAL_ERROR = 0x80020020,
						    /** Internal error */
	AX_CIPHER_LOAD_KEY_ERROR = 0x80020021,
						    /** load key error */
} AX_CIPHER_STS;
...
typedef struct {
	...
	AX_CIPHER_MODE_E workMode;
				 /**< Operating mode */
	uint8_t *pKey;	       /**< Key input */
	uint32_t keySize;		   /**< Key size */
	uint8_t *pIV;	       /**< Initialization vector (IV) */
} ax_cipher_ctrl_s;
...
int32_t ax_cipher_encrypt(AX_CIPHER_HANDLE cipher, uint8_t * src_addr, uint8_t * dest_addr, uint32_t byte_length);
int32_t ax_cipher_decrypt(AX_CIPHER_HANDLE cipher, uint8_t * src_addr, uint8_t * dest_addr, uint32_t byte_length);
int32_t ax_cipher_destroy_handle(AX_CIPHER_HANDLE cipher);
uint32_t ax_cipher_get_random_number(uint8_t * p_random_number, uint32_t size);
int32_t ax_cipher_encrypt_mac_blk(uint8_t * key_addr, uint32_t key_byte_length, uint8_t * src_addr, uint32_t byte_length, uint8_t * mac_addr, uint32_t mac_length);
