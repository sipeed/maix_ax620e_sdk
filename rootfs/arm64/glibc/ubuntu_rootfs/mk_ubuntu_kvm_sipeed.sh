...
cp kvm_requirements.txt $TARGET_ROOTFS_DIR/kvm_requirements.txt
...
apt install -y --allow-unauthenticated vim bash-completion systemd sudo kmod net-tools ethtool resolvconf ifupdown isc-dhcp-server \
        language-pack-en-base htop bc udev ssh rsyslog lrzsz iputils-ping \
        tee-supplicant ifplugd \
        python3 libopus0 portaudio19-dev iperf3 \
        iptables wget curl \
        alsa-utils udhcpd wpasupplicant avahi-daemon chrony \
...
pip install -i https://pypi.mirrors.ustc.edu.cn/simple -r /kvm_requirements.txt

mv /etc/apt/sources.list.bak /etc/apt/sources.list

cd /
rm -rf /root/.cache
rm -rf /kvm_requirements.txt
rm -rf /Python-*
