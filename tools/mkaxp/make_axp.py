...
def print_usage():
    print('usage:')
    print('python3 make_axp.py [-h] -p [-v] -o -x file1 file2 file3 ...')
...
def calc_auth_value(file, is_md5=True):
    print('\tCalculating %s  -->  file: %s, size: %d' %(('md5' if is_md5 else 'crc16'), get_fname(file), os.path.getsize(file)))
...
def update_xml(path, args):
    ...
    for node_img in root.iter(XML_NODE_Img):
        ...
        if check_need_input_file(flag):
            ...
            node_file.text = get_fname(args['imgs'][file_no])
            node_auth = find_child(node_img, XML_NODE_Auth)
            algo = node_auth.get(XML_ATTR_algo)
            if algo and int(algo) > 0:
                node_auth.text = calc_auth_value(args['imgs'][file_no], True if int(algo) == 1 else False)
            file_no += 1
...
def parse_args():
    ...
    opts, args['imgs'] = getopt.getopt(sys.argv[1:], '-h-p:-v:-o:-x:',
    ...
    if file_nums != len(args['imgs']):
        print('[Error] The number of input disk files <{}> is not equal to expected <{}>'.format(len(args['imgs']), file_nums))
        exit(1)

    args['imgs'] = list(map(get_abspath, args['imgs']))
    for img in args['imgs']:
        if not os.path.exists(img):
            print('[Error] <{}> does not exist'.format(img))
            exit(1)
        if not os.path.isfile(img):
            print('[Error] <{}> is not a disk file'.format(img))
            exit(1)
        if os.path.getsize(img) == 0:
            print('[Error] <{}> size is 0'.format(img))
            exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        src_files = copy.deepcopy(args['imgs'])
        src_files.append(args['xml'])
        dst_files = []
        tmp_files = []
        for i in range(len(src_files)):
            dst_file = os.path.join(zip_dir, get_fname(src_files[i]))
            if src_files[i] in tmp_files:
            ...
            if dst_file in dst_files:
                ...
                dst_file = dst_file + '.1'
                args['imgs'][i] = dst_file

            dst_files.append(dst_file)
            tmp_files.append(src_files[i])
