...
def do_spl(in_file, pub_file, prv_file, out_file, out_bk_file, fw_file, capability, part_size):
    ...
    max_img_size = part_size - header_size * 2
    all_img_pack_size = part_size
    all_img_pack_size_bk = part_size
    ...
    img_size = in_size
    boot_bak_flash_addr = part_size
    fw_flash_addr = (header_size*2+in_size + (IMG_ALIGN - 1)) & (~(IMG_ALIGN - 1)) #0x40000
    fw_bak_flash_addr = (part_size + in_size + (IMG_ALIGN - 1)) & (~(IMG_ALIGN - 1))
    ...
    copy_data_by_bytes(file_data, out_data, in_size, header_size*2)
    # calc img check sum
    check_sum = ctypes.c_uint32(0)
    for i in range(header_size*2//4, in_size//4+(header_size*2//4)):
        data = out_data[4*i] + (out_data[4*i + 1] << 8) + (out_data[4*i + 2] << 16) + (out_data[4*i + 3] << 24)
    ...
    copy_data_by_bytes(pub_n, hdr.rsa_key_n, 384)
    ...
    copy_data_by_bytes(signature, hdr.signature, 384, 0, True)
    ...
    packed_data_bk = (ctypes.c_uint8 * (all_img_pack_size_bk))(0)
	#copy header twice
    copy_data_by_bytes(out_data, packed_data, header_size, 0, False, False)
    copy_data_by_bytes(out_data, packed_data, header_size, header_size, False, False)

    # load in file back data
    copy_data_by_bytes(file_data, packed_data, in_size, header_size*2)
    copy_data_by_bytes(file_data, packed_data_bk, in_size)
    # load fw file and back data
    copy_data_by_bytes(fw_data, packed_data, fw_size, fw_flash_addr)
    copy_data_by_bytes(fw_data, packed_data_bk, fw_size, fw_bak_flash_addr - part_size)

    # save data to out file
    try:
        with open(out_file, 'wb') as f:
            f.write(packed_data)
        with open(out_bk_file, 'wb') as f:
            f.write(packed_data_bk)
    except:
...
if __name__ == '__main__':
    ...
    for i in range(1, len(sys.argv)):
        if sys.argv[i] == '-i':
            ...
            out_bk_file = sys.argv[i + 1]
        elif sys.argv[i] == '-fw':
            fw_file = sys.argv[i + 1]
        elif sys.argv[i] == '-cap':
            capability = int(sys.argv[i + 1], 16)
        elif sys.argv[i] == '-partsize':
            part_size = int(sys.argv[i + 1], 16)
    ...
    '''
    ret = do_spl(in_file, pub_file, prv_file, out_file, out_bk_file, fw_file, capability, part_size)
    if ret:
        print('usb back partition , sign complete')
