...
def print_usage():
    print('\nuboot_sign.py -i <uboot_file> -pub <public_key_file(*.pem)> -prv <private_key_file(*.pem)> -o <out_file>')
    print('  -i   :  input original uboot file path, the file size must be less than 111K bytes')
...
class uboot_header(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
        ("check_sum", ctypes.c_int),
        ("magic_data", ctypes.c_int),
        ("img_size", ctypes.c_int),
...
def do_sign(in_file, pub_file, prv_file, out_file):
    ...
    out_size = HEADER_SIZE + in_size

    try:
        with open(in_file, 'rb') as f:
            file_data = f.read()
    except:
        print('[error] load img file failed\n')
        print(sys.exc_info())
        print(traceback.format_exc())
        return False

    # construct uboot_header
    hdr = uboot_header()
    ...
    out_data = (ctypes.c_uint8 * (out_size))(0)
...
if __name__ == '__main__':
    ...
    '''
    ret = do_sign(in_file, pub_file, prv_file, out_file)
    if ret:
        print('sign complete')
