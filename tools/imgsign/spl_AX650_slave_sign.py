...
def do_spl(in_file, pub_file, prv_file, out_file, fw_file, pack_size, capability):
    ...
    all_img_pack_size = (pack_size + (IMG_ALIGN - 1) &(~(IMG_ALIGN - 1)))
    ...
    boot_bak_flash_addr = (header_size*2+in_size + (IMG_ALIGN - 1)) & (~(IMG_ALIGN - 1)) #0x20000
    fw_flash_addr = (boot_bak_flash_addr+in_size + (IMG_ALIGN - 1)) & (~(IMG_ALIGN - 1)) #0x40000
    fw_bak_flash_addr = fw_flash_addr + IMG_ALIGN #0x60000
