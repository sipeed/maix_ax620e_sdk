...
AX_BOOL CIPCBuilder::InitSensor() {
    ...
    AX_U32 nDiscardFrmNum = COptionHelper::GetInstance()->GetDiscardYuvFrmNum();
    ...
    for (AX_U32 i = 0; i < nSnsCnt; i++) {
        ...
        if (tSnsConfig.nDiscardYUVFrmNum == 0 && nDiscardFrmNum > 0) {
            pSensor->SetDiscardYuvFrameNum(nDiscardFrmNum);
        }
        AX_U8 nSnsID = tSnsConfig.nSnsID;
        WEB_CAMERA_ATTR_T tCamera;
        tCamera.bLFHdrSupport = tSnsConfig.bLFHdrSupport;
        tCamera.nSnsMode = tSnsConfig.nSensorMode;
        ...
        if (tSnsConfig.eSensorType == E_SNS_TYPE_C4395) {
            ...
            tCamera.bSnsModeEnable = AX_FALSE;
        } else if (tSnsConfig.eSensorType == E_SNS_TYPE_OS12D40) {
            tCamera.bSnsModeEnable = AX_FALSE;
        } else if (tSnsConfig.eSensorType == E_SNS_TYPE_IMX258) {
            tCamera.bSnsModeEnable = AX_FALSE;
        ...
        tCamera.bPNModeEnable = (nSnsCnt > 2) ? AX_FALSE : AX_TRUE;
        ...
        tCamera.tEZoomAttr.fMaxEZoomRatio = tSnsConfig.tEZoomAttr.fEZoomMaxRatio;
        ...
        SetWebFrameRateOption(tSnsConfig);
#endif
    }
...
AX_BOOL CIPCBuilder::InitIvps() {
    ...
    for (AX_U8 i = 0; i < MAX_IVPS_GROUP_NUM; i++) {
        ...
        // bind sensor
        pIvpsInstance->BindSensorMgr(&m_mgrSensor);
        ...
        if (GetRelationsByDstMod(tDstMod, vecRelations, AX_TRUE)) {
            ...
            for (auto& relation : vecRelations) {
                if (E_PPL_MOD_TYPE_IVPS == relation.tSrcModChn.eModType) {
                    ...
                    // bind sensor
                    pIves->BindSensorMgr(&m_mgrSensor);
#ifndef SLT
                    // web event
                    m_vecWebObs.emplace_back(CObserverMaker::CreateObserver<CWebObserver>(CWebServer::GetInstance()));
...
AX_BOOL CIPCBuilder::InitPoolConfig() {
    ...
    for (AX_U8 i = 0; i < nSnsCnt; i++) {
        if (m_mgrSensor.GetSnsInstance(i)) {
            m_mgrSensor.GetSnsInstance(i)->GetResolution(tAttr[i].nMaxWidth, tAttr[i].nMaxHeight);
...
AX_BOOL CIPCBuilder::UpdateRotation(AX_U8 nSnsID, AX_U8 nRotation) {
    ...
    SendWebResetEvent(nSnsID);

    /* Venc start recv */
    for (auto pInstance : m_vecVencInstance) {
        if (pInstance->GetSensorSrc() == nSnsID) {
            pInstance->StartRecv();
            pInstance->RequestIDR();
        }
    }
    ...
    for (auto pInstance : m_vecVencInstance) {
        if (pInstance->GetSensorSrc() == nSnsID) {
            pInstance->RequestIDR(AX_TRUE);
            CElapsedTimer::mSleep(40);
            pInstance->RequestIDR(AX_TRUE);
        }
    }
...
AX_BOOL CIPCBuilder::DispatchOpr(WEB_REQ_OPERATION_T& tOperation, AX_VOID** pResult /*= nullptr*/) {
    ...
    switch (eOperaType) {
        case E_WEB_OPERATION_TYPE_SNS_MODE: {
            ...
            SENSOR_CONFIG_T tSnsConfig = pSensor->GetSnsConfig();
            SetWebFrameRateOption(tSnsConfig, AX_TRUE);
#endif
            break;
        }
        case E_WEB_OPERATION_TYPE_SNS_RESOLUTION: {
            AX_POOL_FLOORPLAN_T tPrivPoolFloorPlan = m_pPoolConfig->GetPrivPoolFloorPlan();

            m_mgrSensor.SwitchSnsResolution(tOperation.nSnsID, tOperation.tSnsResolution.nSnsWidth, tOperation.tSnsResolution.nSnsHeight, tPrivPoolFloorPlan, m_eLowMemMode);
            break;
        }
        case E_WEB_OPERATION_TYPE_CAMERA_FPS: {
            ...
            LOG_MM_C(PPL, "UpdateSnsFRC[%d] %2f -> %2f ", tOperation.nSnsID, fCurFrameRate, tOperation.tSnsFpsAttr.fSnsFrameRate);
            ...
            for (auto& pInstance : m_vecIvpsInstance) {
                ...
                if (pConfig->nSnsSrc == tOperation.nSnsID) {
                    pInstance->UpdateChnFRC(tOperation.tSnsFpsAttr.fSnsFrameRate);
                }
            }

            for (auto& pInstance : m_vecVencInstance) {
                IPC_MOD_INFO_T tDstMod = {E_PPL_MOD_TYPE_VENC, 0, pInstance->GetChannel()};
                IPC_MOD_INFO_T tPrecedingMod;
                tPrecedingMod.eModType = E_PPL_MOD_TYPE_IVPS;
                if (GetPrecedingMod(tDstMod, tPrecedingMod)) {
                    pInstance->UpdateFrameRate(m_vecIvpsInstance[tPrecedingMod.nGroup]->GetGrpCfg()->arrChnFramerate[tPrecedingMod.nChannel][1]);
                }
            }
            break;
        }
        ...
        case E_WEB_OPERATION_TYPE_EZOOM: {
            ...
            m_mgrSensor.EZoom(tOperation.nSnsID, tOperation.tEZoom.fEZoomRatio);

            /* update web ezoom value */
            tCamera.tEZoomAttr.fEZoomRatio = tOperation.tEZoom.fEZoomRatio;
        ...
        case E_WEB_OPERATION_TYPE_ENC_TYPE: {
            ...
            for (auto& pVideoInstance : m_vecVencInstance) {
                if (pVideoInstance->GetChannel() == tOperation.nChannel) {
                    ...
                    SendWebResetEvent(tOperation.nSnsID);
                    pVideoInstance->StartRecv();
                    pVideoInstance->RequestIDR(AX_TRUE);
                    CElapsedTimer::mSleep(40);
                    pVideoInstance->RequestIDR(AX_TRUE);
                    break;
                }
        ...
        case E_WEB_OPERATION_TYPE_RESOLUTION: {
            ...
            if (GetPrecedingMod(tDstMod, tPrecedingMod)) {
                ...
                m_vecIvpsInstance[tPrecedingMod.nGroup]->UpdateChnResolution(tPrecedingMod.nChannel, tOperation.tResolution.nWidth,
                                                                             tOperation.tResolution.nHeight, tOperation.tResolution.bUpdateIvpsGrp);
                ...
                pVencInstance->RequestIDR(AX_TRUE);
        ...
        case E_WEB_OPERATION_TYPE_RC_INFO: {
            ...
            if (nullptr != pVencInstance) {
                RC_INFO_T tRcInfo;
                AX_VENC_RC_MODE_E eOldRcType = AX_VENC_RC_MODE_BUTT;
                ...
                SendWebResetEvent(tOperation.nSnsID);
                ret = pVencInstance->UpdateRcInfo(tRcInfo, &eOldRcType);
                if (ret) {
        ...
        case E_WEB_OPERATION_TYPE_AI_BODY_AE_ROI: {
            AX_APP_ALGO_HVCFP_PARAM_T stParam = ALGO_HVCFP_PARAM(tOperation.nSnsID);
            ...
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_FACE].bEnable = tOperation.tAiAeRoi.bEnable;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_FACE].nWidth = tOperation.tAiAeRoi.nWidth;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_FACE].nHeight = tOperation.tAiAeRoi.nHeight;
        ...
        case E_WEB_OPERATION_TYPE_AI_VEHICLE_AE_ROI: {
            AX_APP_ALGO_HVCFP_PARAM_T stParam = ALGO_HVCFP_PARAM(tOperation.nSnsID);

            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].bEnable = tOperation.tAiAeRoi.bEnable;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].nWidth = tOperation.tAiAeRoi.nWidth;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].nHeight = tOperation.tAiAeRoi.nHeight;

            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_CYCLE].bEnable = tOperation.tAiAeRoi.bEnable;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_CYCLE].nWidth = tOperation.tAiAeRoi.nWidth;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_CYCLE].nHeight = tOperation.tAiAeRoi.nHeight;

            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_PLATE].bEnable = tOperation.tAiAeRoi.bEnable;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_PLATE].nWidth = tOperation.tAiAeRoi.nWidth;
            stParam.stAeRoiConfig[AX_APP_ALGO_HVCFP_PLATE].nHeight = tOperation.tAiAeRoi.nHeight;
...
AX_S32 CIPCBuilder::APP_SYS_Init() {
    ...
    CMM_PARTITIONS_INFO_T cmmInfo;
    AX_MOD_INFO_T modInfo;
    memset(&modInfo, 0, sizeof(AX_MOD_INFO_T));
    modInfo.enModId = AX_ID_VIN;
    modInfo.s32GrpId = 0;
    modInfo.s32ChnId = 0;
    ...
    AX_BOOL bCmmEnable = COptionHelper::GetInstance()->GetCmmPartitions(cmmInfo);
    if (bCmmEnable) {
        for (AX_S32 i = 0; i < cmmInfo.nCount; i++) {
            modInfo.enModId = (AX_MOD_ID_E)(cmmInfo.partitions[i].nModId);
            modInfo.s32GrpId = cmmInfo.partitions[i].nGrpId;
            modInfo.s32ChnId = cmmInfo.partitions[i].nChnId;
            nRet = AX_SYS_MemSetParam(&modInfo, (AX_S8*)cmmInfo.partitions[i].szName, cmmInfo.partitions[i].nType);
            LOG_MM_C(PPL, "AX_SYS_MemSetParam, name=%s, mod=%d, grp=%d, chn=%d, type=%d, ret=0x%x",\
                cmmInfo.partitions[i].szName, cmmInfo.partitions[i].nModId, cmmInfo.partitions[i].nGrpId, cmmInfo.partitions[i].nChnId, cmmInfo.partitions[i].nType, nRet);
        }
    }
...
AX_VOID CIPCBuilder::SendWebResetEvent(AX_U8 nSnsID) {
#ifndef SLT
    /*notify web restart preview*/
    WEB_EVENTS_DATA_T event;
    event.eType = E_WEB_EVENTS_TYPE_ReStartPreview;
...
AX_VOID CIPCBuilder::SetWebFrameRateOption(const SENSOR_CONFIG_T& tSnsConfig, AX_BOOL bUpdateFrameRate/*= AX_FALSE*/) {
    AX_S32 nScenario = 0;
    if (!CCmdLineParser::GetInstance()->GetScenario(nScenario)) {
        LOG_M_W(PPL, "Load default scenario %d", nScenario);
    }
    if (nScenario == E_APP_SCENARIO_TRIPLE_SENSOR_SWITCH_SINGLE_PIPE) {
        if ((tSnsConfig.eSensorType == E_SNS_TYPE_OS04A10)
                && (tSnsConfig.eSwitchSnsType == E_SNS_SWITCH_SNS_TYPE_SINGLE_PIPE)) {
            CWebOptionHelper::GetInstance()->SetFramerateOptStr(tSnsConfig.nSnsID, "[20]");
        } else if (tSnsConfig.eSensorType == E_SNS_TYPE_SC850SL) {
            string strFrameRate = (tSnsConfig.eSensorMode == AX_SNS_LINEAR_MODE) ? "[20]" : "[15]";
            CWebOptionHelper::GetInstance()->SetFramerateOptStr(tSnsConfig.nSnsID, strFrameRate);

            if (bUpdateFrameRate) {
                WEB_CAMERA_ATTR_T tCamera = CWebOptionHelper::GetInstance()->GetCamera(tSnsConfig.nSnsID);
                /* update current frame rate */
                tCamera.fFramerate = tSnsConfig.fFrameRate;
                CWebOptionHelper::GetInstance()->SetCamera(tSnsConfig.nSnsID, tCamera);

                LOG_MM_I(PPL, "sns[%d] tCamera.fFramerate: %f", tSnsConfig.nSnsID, tCamera.fFramerate);
            }
        }
    }
}
