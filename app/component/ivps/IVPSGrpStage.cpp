...
static constexpr AX_S32 g_ivps_nMinFBCWidthFor4K = 720;
}  // namespace
...
CIVPSGrpStage::CIVPSGrpStage(IVPS_GROUP_CFG_T& tGrpConfig) : CAXStage(IVPS), m_tIvpsGrpCfg(tGrpConfig), m_nIvpsGrp(tGrpConfig.nGrp) {
    memcpy(m_arrCfgChnFramerate, m_tIvpsGrpCfg.arrChnFramerate, sizeof(m_tIvpsGrpCfg.arrChnFramerate));
}
    ...
    if (0 == nChn) {
        ...
        if (tPipelineAttr.tFilter[0][1].bEngage) {
            ...
            LOG_MM_C(IVPS, "GrpFilter1 Reset resolution for ivps (Grp: %d): w: %d, h: %d, s: %d compress[%d,%d].", m_tIvpsGrpCfg.nGrp,
                     ...
                     tPipelineAttr.tFilter[0][1].nDstPicStride, tPipelineAttr.tFilter[0][1].tCompressInfo.enCompressMode,
                     tPipelineAttr.tFilter[0][1].tCompressInfo.u32CompressLevel);
        }
    }
...
AX_BOOL CIVPSGrpStage::UpdateChnResolution(AX_U8 nChn, AX_S32 nWidth, AX_S32 nHeight, AX_BOOL bUpdateIvpsGrp) {
    AX_IVPS_GetPipelineAttr(m_tIvpsGrpCfg.nGrp, &m_tIvpsGrp.tPipelineAttr);
    AX_IVPS_PIPELINE_ATTR_T& tPipelineAttr = m_tIvpsGrp.tPipelineAttr;
...
AX_BOOL CIVPSGrpStage::UpdateChnFRC(AX_F32 fSnsFrameRate) {
    for (AX_U8 i = 0; i < m_tIvpsGrpCfg.nGrpChnNum; i++) {
        AX_U8 nChnFilter = 0;
        ...
        if (m_tIvpsGrpCfg.arrChnEngineType0[i] != AX_IVPS_ENGINE_BUTT) {
            ...
            if (m_arrCfgChnFramerate[i][0] == -1
                && m_arrCfgChnFramerate[i][1] == -1) {
                m_tIvpsGrpCfg.arrChnFramerate[i][0] = fSnsFrameRate;
                m_tIvpsGrpCfg.arrChnFramerate[i][1] = fSnsFrameRate;

                tChnAttr.tFRC.fSrcFrameRate = fSnsFrameRate;
                tChnAttr.tFRC.fDstFrameRate = fSnsFrameRate;
                bFrcUpdate = AX_TRUE;
            } else if (m_arrCfgChnFramerate[i][0] == -1) {
                m_tIvpsGrpCfg.arrChnFramerate[i][0] = fSnsFrameRate;
                tChnAttr.tFRC.fSrcFrameRate = fSnsFrameRate;
                bFrcUpdate = AX_TRUE;
            } else if (m_arrCfgChnFramerate[i][1] == -1) {
                m_tIvpsGrpCfg.arrChnFramerate[i][1] = fSnsFrameRate;
                tChnAttr.tFRC.fDstFrameRate = fSnsFrameRate;
                bFrcUpdate = AX_TRUE;
            }
            ...
            if (bFrcUpdate) {
                LOG_MM_C(IVPS, "nGrp:%d, nChn:%d, filter:%d, [%f:%f]", m_tIvpsGrpCfg.nGrp, i, nChnFilter, tChnAttr.tFRC.fSrcFrameRate,
...
AX_VOID CIVPSGrpStage::UpdateCompressInfo(AX_FRAME_COMPRESS_INFO_T& tCompressInfo, AX_U16 nChnWidth) {
    if (m_pSensorMgr) {
        CBaseSensor* pSensorInstance = m_pSensorMgr->GetSnsInstance(m_tIvpsGrpCfg.nSnsSrc);

        if (pSensorInstance) {
            AX_U32 nWidth = 0;
            AX_U32 nHeight = 0;

            pSensorInstance->GetResolution(nWidth, nHeight);

            if (nChnWidth < g_ivps_nMinFBCWidth
                || (nWidth >= 4096 && nChnWidth <= g_ivps_nMinFBCWidthFor4K)) {
