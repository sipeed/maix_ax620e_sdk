...
typedef struct _IVPS_GROUP_CFG_T {
    ...
    AX_BOOL bArrChnVoRectFlag[IVPS_MAX_CHANNEL_PER_GROUP]; /* VoRect flag for ivps out channels to next module. */
...
public:
    ...
    AX_BOOL UpdateChnResolution(AX_U8 nChn, AX_S32 nWidth, AX_S32 nHeight, AX_BOOL bUpdateIvpsGrp = AX_FALSE);
    AX_BOOL UpdateRotation(AX_IVPS_ROTATION_E eRotation);
    AX_BOOL UpdateChnFRC(AX_F32 fSnsFrameRate);
...
private:
    ...
    AX_F32 m_arrCfgChnFramerate[IVPS_MAX_CHANNEL_PER_GROUP][2];
