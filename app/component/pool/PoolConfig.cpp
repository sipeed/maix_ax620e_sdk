    ...
    for (auto& v : mapKeys) {
        ...
        if (strKey.find("RAW") != std::string::npos) {
            ...
            if (sscanf(&strKey[0], "RAW0_%d", &nPixFmt) > 0) {
                ...
                nSnsId = 1;
            } else if (sscanf(&strKey[0], "RAW2_%d", &nPixFmt) > 0) {
                nSnsId = 2;
            } else {
            ...
            if (!AddPrivVinBlock("RAW", nBlkSize, nMetaSize, nBlkCnt)) {
                return AX_FALSE;
                    ...
                    || strKey.find("YUV2_MAX") != std::string::npos) {
            ...
            if (strKey.find("YUV0_MAX") != std::string::npos) {
                ...
                nSnsId = 1;
            } else if (strKey.find("YUV2_MAX") != std::string::npos) {
                nSnsId = 2;
            } else {
                nSnsId = 0;
            }
            ...
            if (sscanf(&strKey[0], "YUV0_%dx%d", &nWidth, &nHeight) > 0) {
                ...
                nSnsId = 1;
            } else if (sscanf(&strKey[0], "YUV2_%dx%d", &nWidth, &nHeight) > 0) {
                nSnsId = 2;
            } else {
...
AX_S32 CPoolConfig::InitPrivatePool() {
    ...
    for (AX_U32 k = 0; k < m_nPrivPoolCount; k++) {
        LOG_M(POOL, "Priv Pool[%d] BlkSize:%lld, BlkCnt:%d, MetaSize:%lld", k, m_tPrivPoolFloorPlan.CommPool[k].BlkSize,
...
const AX_POOL_FLOORPLAN_T& CPoolConfig::GetPrivPoolFloorPlan() {
    return m_tPrivPoolFloorPlan;
}
