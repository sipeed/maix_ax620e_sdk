...
#define MAX_WS_CONN_NUM (16)

#define WS_EVENTS_CHANNEL (MAX_WS_CONN_NUM - 1)
#define WS_CAPTURE_CHANNEL (MAX_WS_CONN_NUM - 3)
#define MAX_EVENTS_CHN_SIZE (256)
#define AX_WEB_MAX_PREV_SNS_NUM (3)
...
typedef enum {
    ...
    E_WEB_OPERATION_TYPE_SNS_RESOLUTION,
    ...
    E_WEB_OPERATION_TYPE_CAPTURE_ENABLE,
    E_WEB_OPERATION_TYPE_AI_ENABLE,
    E_WEB_OPERATION_TYPE_AI_PUSH_MODE,
    E_WEB_OPERATION_TYPE_AI_EVENT,
    E_WEB_OPERATION_TYPE_AI_BODY_AE_ROI,
    E_WEB_OPERATION_TYPE_AI_VEHICLE_AE_ROI,
    ...
    E_WEB_OPERATION_TYPE_EZOOM,
    E_WEB_OPERATION_TYPE_DIS_ATTR,
    E_WEB_OPERATION_TYPE_SWITCH_3A_SYNC,
...
typedef struct _WEB_OPR_SNS_RESOLUTION_T {
    AX_U32 nSnsWidth;
    AX_U32 nSnsHeight;

    _WEB_OPR_SNS_RESOLUTION_T() {
        nSnsWidth = 0;
        nSnsHeight = 0;
    }
} WEB_OPR_SNS_RESOLUTION_T;

typedef struct _WEB_OPR_RESOLUTION {
    AX_U8 nEncoderType; /* 0: H264; 1: MJPEG; 2: H265 */
    AX_U32 nWidth;
    AX_U32 nHeight;
    AX_BOOL bUpdateIvpsGrp;

    _WEB_OPR_RESOLUTION() {
        nEncoderType = 0;
        nWidth = 0;
        nHeight = 0;
        bUpdateIvpsGrp = AX_FALSE;
    }
...
typedef struct _WEB_OPR_AI_AE_ROI {
    AX_BOOL bEnable;
    AX_U32 nWidth;
    AX_U32 nHeight;
    ...
    _WEB_OPR_AI_AE_ROI() {
        ...
        nWidth = 100;
        nHeight = 100;
        eMode = AX_APP_ALGO_AE_ROI_MODE_BEST;
    }
...
typedef struct _WEB_OPR_DIS_ATTR_T {
    AX_BOOL bDisEnable{AX_FALSE};
} WEB_OPR_DIS_ATTR_T;
...
typedef struct _WEB_OPR_EZOOM_ATTR_T {
    AX_F32 fEZoomRatio{1.0};
    AX_F32 fMaxEZoomRatio{AX_EZOOM_MAX};
    AX_S32 nCenterOffsetX{0};
    AX_S32 nCenterOffsetY{0};
} WEB_OPR_EZOOM_ATTR_T;
...
typedef struct _WEB_REQ_OPERATION_T {
    ...
    union {
        ...
        WEB_OPR_AI_AE_ROI_T tAiAeRoi;
        WEB_OPR_OSD_ATTR_T tOsdAttr;
...
typedef struct _AI_EVENTS_AED_INFO_T {
    AX_CHAR szDisplay[64];
    AX_S32 nDb;
    AX_U64 nReserved;
    _AI_EVENTS_AED_INFO_T() {
        memset(szDisplay, 0, sizeof(szDisplay));
        nDb = 0;
        nReserved = 0;
    }
} AI_EVENTS_AED_INFO_T;
...
public:
    ...
    AX_VOID SendPreviewData(AX_U8 nUniChn, AX_VENC_PACK_T* pVencPack);
    ...
    AX_VOID SendCaptureData(AX_U8 nSnsID, AX_U8 nUniChn, AX_VOID* data, AX_U32 size, AX_U64 nPts = 0, AX_BOOL bIFrame = AX_TRUE,
                            JPEG_DATA_INFO_T* pJpegInfo = nullptr);
    AX_VOID SendSnapshotData(AX_VOID* data, AX_U32 size, AX_VOID* conn);
...
private:
    ...
    std::string FormatAedEventsJson(WEB_EVENTS_DATA_T* pEvent);
...
private:
    ...
    std::thread* m_pStatucCheckThread{nullptr};
    std::mutex m_mtxConnStatus;

    AX_BOOL m_bStatusCheckStarted{AX_FALSE};
    std::mutex m_mtxStatusCheck;
    std::condition_variable m_cvStatusCheck;

    AX_S8 m_nCaptureChannel{WS_CAPTURE_CHANNEL};
