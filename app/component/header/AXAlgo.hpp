...
#define AX_APP_ALGO_DETECT_DEFAULT_TRACK_STATUS (AX_TRUE)
#define AX_APP_ALGO_DETECT_DEFAULT_PUSH_STATUS (AX_TRUE)
#define AX_APP_ALGO_DETECT_DEFAULT_PANORAMA_STATUS (AX_FALSE)
#define AX_APP_ALGO_DETECT_DEFAULT_PUSH_ATTR_ALWAYS (AX_TRUE)
#define AX_APP_ALGO_DETECT_DEFAULT_OBJECT_FLITER (0x00)
#define AX_APP_ALGO_DETECT_DEFAULT_OBJECT_PUSH_FLITER (0x00)
#define AX_APP_ALGO_DETECT_DEFAULT_ANALAYZER (0x00)

#define AX_APP_ALGO_DETECT_DEFAULT_MODEL_PATH ("/opt/etc/skelModels")

#define AX_APP_ALGO_IVES_DEFAULT_FRAMERATE (-1)
...
typedef enum axAPP_ALGO_AE_ROI_MODE_E {
    AX_APP_ALGO_AE_ROI_MODE_BEST = 1,
    AX_APP_ALGO_AE_ROI_MODE_ENTIRE = 2,
    AX_APP_ALGO_AE_ROI_MODE_BUTT
} AX_APP_ALGO_AE_ROI_MODE_E;
...
typedef struct axAPP_POLYGON_T {
    AX_U32 nPointNum;
    AX_APP_POINT_T stPoints[AX_SKEL_ROI_POINT_MAX];
...
typedef struct axAPP_ALGO_AE_ROI_CONFIG_T {
    AX_BOOL bEnable;
    AX_U32 nWidth;
    AX_U32 nHeight;
    AX_APP_ALGO_AE_ROI_MODE_E eMode;
} AX_APP_ALGO_AE_ROI_CONFIG_T, *AX_APP_ALGO_AE_ROI_CONFIG_PTR;

// algorithm hvcfp parameter
typedef struct axAPP_ALGO_HVCFP_PARAM_T {
    AX_BOOL bEnable;
    AX_S32 nFramerate;
    AX_S32 nObjectTargetFlag;      // 1 << AX_APP_ALGO_HVCFP_TYPE_E
    AX_S32 nObjectPushTargetFlag;  // 1 << AX_APP_ALGO_HVCFP_TYPE_E
    AX_S32 nObjectAnalyzerFlag;    // 1 << AX_SKEL_ANALYZER_ATTR_E
    AX_BOOL bTrackActive;
    AX_BOOL bPushActive;
    ...
    AX_APP_ALGO_AE_ROI_CONFIG_T stAeRoiConfig[AX_APP_ALGO_HVCFP_TYPE_BUTT];
...
// algorithm ives parameter
typedef struct axAPP_ALGO_IVES_PARAM_T {
    AX_S32 nFramerate;
} AX_APP_ALGO_IVES_PARAM_T, *AX_APP_ALGO_IVES_PARAM_PTR;
...
typedef struct axAPP_ALGO_PARAM_T {
    ...
    AX_APP_ALGO_IVES_PARAM_T stIvesParam;
...
typedef struct axAPP_ALGO_AE_ROI_ITEM_T {
    AX_U64 u64TrackId;
    AX_F32 fConfidence;
    AX_APP_ALGO_HVCFP_TYPE_E eType;
...
typedef AX_S32 (*AX_APP_VIDEO_ALGO_CAPTUREFACERECOGNIZE_CALLBACK)(AX_S32 nSnsId,
                                                                  const AX_APP_ALGO_CAPTURE_FACE_RECOGNIZE_RESULT_PTR pstResult);
...
static inline AxAeDetectObjectCategory AX_APP_ALGO_GET_AE_ROI_OBJECT_CATEGORY(const AX_APP_ALGO_HVCFP_TYPE_E &eType) {
    switch (eType) {
    case AX_APP_ALGO_HVCFP_BODY:
        return AXAE_BODY_ROI;
    case AX_APP_ALGO_HVCFP_VEHICLE:
        return AXAE_VEHICLE_ROI;
    case AX_APP_ALGO_HVCFP_CYCLE:
        return AXAE_CYCLE_ROI;
    case AX_APP_ALGO_HVCFP_FACE:
        return AXAE_FACE_ROI;
    case AX_APP_ALGO_HVCFP_PLATE:
