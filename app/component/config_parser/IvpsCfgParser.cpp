...
AX_BOOL CIvpsCfgParser::ParseJson(picojson::object &objJsonRoot, std::map<AX_U8, IVPS_GROUP_CFG_T> &mapGrpSetting) {
    ...
    do {
        ...
        for (size_t i = 0; i < arrGrpSetting.size(); i++) {
            ...
            if (objGroupInfo.end() != objGroupInfo.find("chn_vorect_flag")) {
                picojson::array &arryChnVoRectFlag = objGroupInfo["chn_vorect_flag"].get<picojson::array>();
                for (size_t i = 0; i < arryChnVoRectFlag.size(); i++) {
                    AX_S32 nVoRect = arryChnVoRectFlag[i].get<double>();
                    tOutConfig.bArrChnVoRectFlag[i] = nVoRect ? AX_TRUE : AX_FALSE;
                }
            }
