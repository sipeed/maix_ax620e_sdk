...
typedef struct _APP_CROP_RECT_INFO_ATTR_T {
    AX_U8          nSnsId{0};
    AX_U8          bValid{0};
    AX_WIN_AREA_T  tCropRect{0};
} APP_CROP_RECT_INFO_ATTR_T;
...
public:
    ...
    AX_BOOL ChangeSnsResolution(AX_U32 nWidth, AX_U32 nHeight);
    AX_BOOL ChangeHdrMode(AX_U32 nSnsMode, AX_U8 nHdrRatio);
    AX_BOOL ChangeSnsMirrorFlip(AX_BOOL bMirror, AX_BOOL bFlip);
    AX_BOOL SetAeRoiAttr(const SENSOR_AE_ROI_ATTR_T& tAttr);
    ...
    AX_VOID SetDiscardYuvFrameNum(AX_U32 nFrmNum);

    AX_VOID SetSubSwitchSnsData(CBaseSensor* pSwitchSubSns);

    AX_VOID SetSnsStartStatus(AX_BOOL bStarted);

    AX_SENSOR_REGISTER_FUNC_T* GetSensorObj() {
        return m_pSnsObj;
    }

    AX_BOOL SetEZoomAeAwbIQ(AX_U8 nSnsId, AX_U8 nPipeId);
...
protected:
    virtual AX_BOOL RegisterSensor(AX_U8 nPipe, AX_SENSOR_REGISTER_FUNC_T* pSnsObj, AX_U8 nBusType, AX_U8 nDevNode, AX_U8 nI2cAddr);
    virtual AX_BOOL UnRegisterSensor(AX_U8 nPipe);
    virtual AX_BOOL ResetSensorObj(AX_U8 nPipe, AX_SENSOR_REGISTER_FUNC_T* pSnsObj, AX_U8 nRestGpio);
...
private:
    ...
    std::string GetTuningBinKey();
    AX_BOOL LoadBinParams(AX_U8 nPipeId, const vector<string>& vecPipeTuningBin);
    AX_U32 GetPipeCount(); // Total pipe count that may include pipe belong to switch sensor, be used for open/close
    AX_BOOL EnableMipiSwitch();
    AX_BOOL RegisterSinglePipeSensors(AX_U8 nPipe);

    AX_BOOL Check3ARoi(AX_U32 nImgWidth, AX_U32 nImgHeight, const AX_ISP_IQ_AE_STAT_ROI_T *pRoi);
    AX_BOOL Get3ARoi(AX_U32 nImgWidth, AX_U32 nImgHeight, const AX_WIN_AREA_T *pCropRect, AX_ISP_IQ_AE_STAT_PARAM_T *ptAeParam, AX_ISP_IQ_AWB_STAT_PARAM_T *ptAwbParam);

    AX_BOOL InitEZoom();
    AX_BOOL GetEZoomCropRect(AX_U32 nWidth, AX_U32 nHeight, AX_S16 nCenterOffsetX, AX_S16 nCenterOffsetY, AX_F32 fEZoomRatio, AX_F32 fEZoomMaxRatio, AX_WIN_AREA_T& tCropRect);
    AX_BOOL GetChnCrop(AX_U8 nPipId, AX_U8 nSnsId, AX_VIN_CROP_INFO_T* pCropInfo);
    AX_BOOL SetChnCrop(AX_U8 nPipId, AX_U8 nSnsId, const AX_VIN_CROP_INFO_T* pCropInfo);
    AX_VOID SetEZoomCropRect(AX_U8 nSnsId, AX_U32 nImgWidth, AX_U32 nImgHeight, AX_WIN_AREA_T* pCropRect, AX_VIN_ROTATION_E eRotation);
    AX_BOOL GetDstCenterOffset(AX_U8 nIvpsGrp, AX_U32 nWidth, AX_U32 nHeight, AX_S16 &nCenterOffsetX, AX_S16 &nCenterOffsetY);
...
protected:
    ...
    SENSOR_SWITCH_SUB_SNS_INFO_T m_tSwitchSubSnsInfo;

    std::mutex m_mtxCrop;
    APP_CROP_RECT_INFO_ATTR_T m_tCropRectInfo;
    AX_WIN_AREA_T m_tPreCropRect[MAX_PIPE_PER_DEVICE]{0};
};
