...
CMIS2032::CMIS2032(SENSOR_CONFIG_T tSensorConfig) : CBaseSensor(tSensorConfig) {
...
CMIS2032::~CMIS2032(AX_VOID) {
}

AX_VOID CMIS2032::InitSnsLibraryInfo(AX_VOID) {
#ifndef USE_STATIC_LIBS
    if (m_tSnsCfg.m_tSnsLibInfo.strLibName.size() == 0) {
        m_tSnsCfg.m_tSnsLibInfo.strLibName = "libsns_mis2032.so";
    }

    if (m_tSnsCfg.m_tSnsLibInfo.strObjName.size() == 0) {
        m_tSnsCfg.m_tSnsLibInfo.strObjName = "gSnsmis2032Obj";
    }
#else
    extern AX_SENSOR_REGISTER_FUNC_T gSnsmis2032Obj;
    m_pSnsObj = &gSnsmis2032Obj;
#endif  // USE_STATIC_LIBS
}

AX_VOID CMIS2032::InitSnsAttr() {
...
AX_VOID CMIS2032::InitMipiRxAttr() {
    ...
    m_tMipiRxDev.tMipiAttr.nDataRate = IS_SNS_HDR_MODE(m_tSnsCfg.eSensorMode) ? 756 : 378;
...
AX_VOID CMIS2032::InitAbilities() {
