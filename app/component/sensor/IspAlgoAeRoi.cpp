...
static AX_ISP_AE_FACE_RECT_INPUT_PARAM_T AeRoiBoxConvert(AX_SKEL_RECT_T tRect,
    ...
    if (fEZoomRatio > 0) {
        ...
        tCropRect.nWidth = nStepW * (AX_EZOOM_MAX - fEZoomRatio + 1);
        tCropRect.nHeight = nStepH * (AX_EZOOM_MAX - fEZoomRatio + 1);
        tCropRect.nStartX = (nOriginalWidth - tCropRect.nWidth) / 2;
        tCropRect.nStartY = (nOriginalHeight - tCropRect.nHeight) / 2;

        tRect.fX = tRect.fX * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
        tRect.fY = tRect.fY * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
        tRect.fW = tRect.fW * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
        tRect.fH = tRect.fH * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
...
static AX_VOID AeRoiBoxConvert(std::vector<AX_APP_ALGO_AE_ROI_ITEM_T>& stVecItem, CBaseSensor* pSensorInst) {
    ...
    AX_F32 fEZoomRatio = pSensorInst->GetSnsConfig().tEZoomAttr.fEZoomRatio;
    ...
    pSensorInst->GetResolution(nImageWidth, nImageHeight);
    ...
    if (fEZoomRatio > 0) {
      ...
      fCropWidth = fCropStepW * (AX_EZOOM_MAX - fEZoomRatio + 1);
      fCropHeight = fCropStepH * (AX_EZOOM_MAX - fEZoomRatio + 1);
      fCropStartX = (1 - fCropWidth) / 2;
      fCropStartY = (1 - fCropHeight) / 2;
    }

    if (bMirror || bFlip || eRotation != AX_ROTATION_0 || fEZoomRatio > 0) {
        for (auto &stItem : stVecItem) {
            if (fEZoomRatio > 0) {
              stItem.tBox.fX = stItem.tBox.fX * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
              stItem.tBox.fY = stItem.tBox.fY * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
              stItem.tBox.fW = stItem.tBox.fW * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
              stItem.tBox.fH = stItem.tBox.fH * (AX_EZOOM_MAX - fEZoomRatio + 1) / AX_EZOOM_MAX;
...
AX_BOOL CIspAlgoAeRoi::StartMonitor() {
    if (m_tAttr.bFaceAeRoiEnable
        || m_tAttr.bVehicleAeRoiEnable) {
...
AX_S32 CIspAlgoAeRoi::Monitor(AX_VOID* pArg) {
    SENSOR_AE_ROI_ATTR_T tAttr = m_tAttr;

    if (m_pSensorInst) {
        m_pSensorInst->SetAeRoiAttr(tAttr);
    }
    ...
    while (m_Thread.IsRunning()) {
...

        {
            ...
            if (m_tStatus.bUpdateTrigger) {
                ...
                if (tAttr.bFaceAeRoiEnable != m_tAttr.bFaceAeRoiEnable
                    || tAttr.bVehicleAeRoiEnable != m_tAttr.bVehicleAeRoiEnable) {
                    tAttr = m_tAttr;
