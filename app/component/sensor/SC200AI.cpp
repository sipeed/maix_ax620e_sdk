...
#define SENSOR_ATTR_WIDTH (1920)
#define SENSOR_ATTR_HEIGHT (1080)

CSC200AI::CSC200AI(SENSOR_CONFIG_T tSensorConfig) : CBaseSensor(tSensorConfig) {
...
CSC200AI::~CSC200AI(AX_VOID) {
}

AX_VOID CSC200AI::InitSnsLibraryInfo(AX_VOID) {
#ifndef USE_STATIC_LIBS
    if (m_tSnsCfg.m_tSnsLibInfo.strLibName.size() == 0) {
        m_tSnsCfg.m_tSnsLibInfo.strLibName = "libsns_sc200ai.so";
    }

    if (m_tSnsCfg.m_tSnsLibInfo.strObjName.size() == 0) {
        m_tSnsCfg.m_tSnsLibInfo.strObjName = "gSnssc200aiObj";
    }
#else
    extern AX_SENSOR_REGISTER_FUNC_T gSnssc200aiObj;
    m_pSnsObj = &gSnssc200aiObj;
#endif  // USE_STATIC_LIBS
}

AX_VOID CSC200AI::InitSnsAttr() {
...
AX_VOID CSC200AI::InitMipiRxAttr() {
    ...
    m_tMipiRxDev.tMipiAttr.nDataRate = IS_SNS_HDR_MODE(m_tSnsCfg.eSensorMode) ? 800 : 400;
...
AX_VOID CSC200AI::InitAbilities() {
