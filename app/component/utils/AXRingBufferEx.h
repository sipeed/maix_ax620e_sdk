...
public:
    ...
    AX_BOOL Put(CAXRingElementEx& element) {
        ...
        if (m_nTail < nHead) {
            LOG_MM_E(AXRINGEX, "[%s] ---, tail(%llu) must not be smaller than head(%llu)", m_szName, m_nTail, nHead);
            return AX_FALSE;
        }

        if ((AX_U32)(m_nTail - nHead) > m_nRingSize) {
            LOG_MM_E(AXRINGEX, "[%s] ---, tail(%llu) - head(%llu) is bigger than ringsize(%u)", m_szName, m_nTail, nHead, m_nRingSize);
            return AX_FALSE;
        }
            ...
            if (element.bIFrame) {
                ...
                do {
                    ...
                    if (nPreEleSize == 0 || nPreEleSize != pEle->nTotalSize || nPreEleSize > m_nRingSize || pEle->pBuf2 != m_pRingBuf) {
                        LOG_MM_E(AXRINGEX, "[%s] previous element size (%u, %u) is invalid", m_szName, nPreEleSize, pEle->nTotalSize);
                        break;
                    }
    ...
    CAXRingElementEx* Get() {
        ...
        if (!CheckElement(element)) {
            LOG_MM_E(AXRINGEX, "[%s] element(%p) is invalid", m_szName, element);
            return nullptr;
        }
    ...
    AX_VOID Free(CAXRingElementEx* ele, AX_BOOL bForce = AX_FALSE) {
        ...
        if (!CheckElement(ele)) {
            LOG_MM_E(AXRINGEX, "[%s] element(%p) is invalid", m_szName, ele);
            return;
        }
...
private:
    AX_BOOL CheckElement(CAXRingElementEx* ele) {
        ...
        if (ele->pBuf2 != m_pRingBuf) {
            LOG_MM_E(AXRINGEX, "[%s] element pBuf2(%p) is not equal ringptr(%p)", m_szName, ele->pBuf2, m_pRingBuf);
            return AX_FALSE;
        }
