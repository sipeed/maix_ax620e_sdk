...
AX_BOOL CMD::Startup(AX_S32 nSnsID, AX_U32 nWidth, AX_U32 nHeight) {
    std::lock_guard<std::mutex> lck(m_mutx);
    LOG_MM_I(MD, "+++nSnsID:%d, nWidth:%d, nHeight:%d", nSnsID, nWidth, nHeight);
        ...
        if (m) {
            if (IS_MDCHN_CREATE(m->stAttr.mdChn)) {
                ...
                if (0 != ret) {
                    ...
                    LOG_MM_D(MD, "AX_IVES_MD_DestoryChn[%d]", m->stAttr.mdChn);
                }
            }
...
AX_S32 CMD::AddArea(AX_U32 x, AX_U32 y, AX_U32 w, AX_U32 h, AX_U32 mbW, AX_U32 mbH, AX_S32 nThrdY, AX_S32 nConfidenceY, AX_U32 nWidth,
                    AX_U32 nHeight, AX_S32 nSnsID) {
    std::lock_guard<std::mutex> lck(m_mutx);
    LOG_MM_I(MD, "+++ x:%d,y:%d, w:%d,h:%d,mnW:%d, mbH:%d, nWidth:%d, nHeight:%d", x, y, w, h, mbW, mbH, nWidth, nHeight);
    ...
    MD_AREA_ATTR_T *pArea = (MD_AREA_ATTR_T *)AX_MALLOC(sizeof(MD_AREA_ATTR_T));
    if (!pArea) {
...
const std::vector<AX_U8> &CMD::ProcessFrame(const AX_U32 nSnsID, const CAXFrame *pFrame) {
    ...
    for (AX_U32 i = 0; i < nAreaCount; ++i) {
        ...
        if (m) {
            ...
            AX_MD_MB_SAD_T stSad{0, nullptr};
            AX_IVES_CCBLOB_T stBlob;
            ret = AX_IVES_MD_Process(m->stAttr.mdChn, pstImg, &stThrs, &stSad, &stBlob);
            if (0 != ret) {
...
AX_VOID CMD::UpdateResolution(AX_U8 nRotation, AX_U32 nFrmW, AX_U32 nFrmH) {
    LOG_MM_I(MD, "+++ FrmW:%d, FrmH:%d", nFrmW, nFrmH);

    m_mdImgW = nFrmW;
    m_mdImgH = nFrmH;
    m_nRotation = nRotation;

    AX_APP_ALGO_MOTION_PARAM_T stParam = ALGO_MD_PARAM(m_nSnsID);
    SetConfig(stParam);

    LOG_MM_I(MD, "---");
}

AX_VOID CMD::AddArea(const AX_APP_ALGO_MOTION_PARAM_T &stConfig) {
    if (stConfig.nRegionSize == 1 && (AX_U32)stConfig.stRegions[0].stRect.fX == 0 && (AX_U32)stConfig.stRegions[0].stRect.fY == 0 &&
        (AX_U32)stConfig.stRegions[0].stRect.fW == 0 && (AX_U32)stConfig.stRegions[0].stRect.fH == 0) {
        AX_U32 nAreaW = m_mdImgW / 2;
        AX_U32 nAreaH = m_mdImgH / 2;

        AX_U32 nX = m_mdImgW / 2 - nAreaW / 2;
        AX_U32 nY = m_mdImgH / 2 - nAreaH / 2;

        AX_U32 nMbWidth = stConfig.stRegions[0].nMbWidth;
        AX_U32 nMbHeight = stConfig.stRegions[0].nMbHeight;

        // nRotation 0: 0; 1: 90; 2: 180; 3: 270
        if (1 == m_nRotation || 3 == m_nRotation) {
            std::swap(nMbWidth, nMbHeight);
        }

        AddArea(nX, nY, nAreaW, nAreaH, nMbWidth, nMbHeight,
                (AX_S32)stConfig.stRegions[0].fThresholdY, (AX_S32)stConfig.stRegions[0].fConfidence, m_mdImgW, m_mdImgH, m_nSnsID);
    } else {
        for (AX_U32 i = 0; i < stConfig.nRegionSize; i++) {
            AX_U32 nAreaW = (AX_U32)stConfig.stRegions[i].stRect.fW;
            AX_U32 nAreaH = (AX_U32)stConfig.stRegions[i].stRect.fH;

            AX_U32 nX = (AX_U32)stConfig.stRegions[i].stRect.fX;
            AX_U32 nY = (AX_U32)stConfig.stRegions[i].stRect.fY;

            AX_U32 nMbWidth = stConfig.stRegions[i].nMbWidth;
            AX_U32 nMbHeight = stConfig.stRegions[i].nMbHeight;

            // nRotation 0: 0; 1: 90; 2: 180; 3: 270
            if (1 == m_nRotation || 3 == m_nRotation) {
                std::swap(nAreaW, nAreaH);
                std::swap(nX, nY);
                std::swap(nMbWidth, nMbHeight);
            }

            AddArea(nX, nY, nAreaW, nAreaH, nMbWidth, nMbHeight,
                    (AX_S32)stConfig.stRegions[i].fThresholdY, (AX_S32)stConfig.stRegions[i].fConfidence, m_mdImgW, m_mdImgH, m_nSnsID);
        }
    }
}
