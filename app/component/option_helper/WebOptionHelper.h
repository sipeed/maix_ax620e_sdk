...
typedef struct _WEB_CAMERA_ATTR_T {
    ...
    _WEB_CAMERA_ATTR_T() {
        nSnsMode = SNS_MODE_SDR;  /* 1: SDR; 2: HDR; 3: LF-SDR; 4: LF-HDR*/
        nSnsResolution = 0;
...
typedef struct _AI_ATTR_T {
    ...
    AI_AE_ROI_OPTION_T tAeRoiVehicle;
...
public:
    ...
    std::string SetFramerateOptStr(AX_U8 nSnsID, string strFrameRate);
    std::string GetSnsResolutionOptStr(AX_U8 nSnsID);
    AX_BOOL GetSnsResolution(AX_U8 nSnsID, AX_U8 nSnsResolution, WEB_OPR_SNS_RESOLUTION_T &tSnsResolution);
    ...
    std::string GetDetectModelStr(AX_U8 nSnsID);
...
private:
    ...
    std::map<AX_U8, std::string> m_mapSnsResolutionOpt;
