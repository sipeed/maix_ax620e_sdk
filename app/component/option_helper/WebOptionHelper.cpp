...
AX_BOOL CWebOptionHelper::InitOnce() {
    ...
    m_mapCapabilities["ai"] = (nSnsCnt == 3) ? 0 : 1;
#ifndef SLT
    ...
    if ((nSnsCnt == nDoubleSensor) && IS_AX620Q) {
    ...
    if (IS_AX620Q) {
        ...
        m_mapSns2FramerateOpt[E_SNS_TYPE_OS12D40] = "[15]";
        m_mapSns2FramerateOpt[E_SNS_TYPE_IMX258] = "[10]";
    } else {
    ...
    if (nSnsCnt == 3) { // Triple sensor
        m_mapSns2FramerateOpt[E_SNS_TYPE_OS04A10] = "[15]";
    }

    if (nSnsCnt < nDoubleSensor) {
        m_mapSnsResolutionOpt[E_SNS_TYPE_OS12D40] = "[12M(4480x2512), 3M(2240x1256)]";
    }
...
AX_BOOL CWebOptionHelper::InitAiAttr(AX_U8 nSnsID) {
    ...
    m_mapSns2AiAttr[nSnsID].eDetectModel = (AX_SKEL_PPL_E)CAlgoOptionHelper::GetInstance()->GetDetectAlgoType(nSnsID);
    ...
    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.bEnable = ALGO_HVCFP_PARAM(nSnsID).stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].bEnable;
    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nWidth = ALGO_HVCFP_PARAM(nSnsID).stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].nWidth;
    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nHeight = ALGO_HVCFP_PARAM(nSnsID).stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].nHeight;
    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.eMode = ALGO_HVCFP_PARAM(nSnsID).stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].eMode;
...
MprJson* CWebOptionHelper::GetCameraJson(AX_U8 nSnsID) {
    ...
    WRITE_JSON_INT(json, "sns_resolution", attr.nSnsResolution);
...
std::string CWebOptionHelper::SetFramerateOptStr(AX_U8 nSnsID, string strFrameRate) {
    return m_mapSns2FramerateOpt[m_mapSnsID2Type[nSnsID]] = strFrameRate;
}

std::string CWebOptionHelper::GetSnsResolutionOptStr(AX_U8 nSnsID) {
    if (m_mapSnsResolutionOpt[m_mapSnsID2Type[nSnsID]].size() > 0) {
        return m_mapSnsResolutionOpt[m_mapSnsID2Type[nSnsID]];
    }

    return "[]";
}

AX_BOOL CWebOptionHelper::GetSnsResolution(AX_U8 nSnsID, AX_U8 nSnsResolution, WEB_OPR_SNS_RESOLUTION_T &tSnsResolution) {
    std::string strSnsResOptList = m_mapSnsResolutionOpt[m_mapSnsID2Type[nSnsID]];
    std::map<AX_U8, WEB_OPR_SNS_RESOLUTION_T> m_mapResOpts;

    const AX_CHAR delimiter[] = ",";

    AX_U8 Index = 0;

    AX_CHAR *str = strdup(strSnsResOptList.c_str());
    if (!str) {
        return AX_FALSE;
    }

    AX_CHAR *token = strtok(str, delimiter);
    ...
    while (token != NULL) {
        ...
        if (token) {
            ...
            sscanf(ptr, "%dx%d", &nWidth, &nHeight);

            m_mapResOpts[Index].nSnsWidth = (AX_U32)nWidth;
            m_mapResOpts[Index].nSnsHeight = (AX_U32)nHeight;
    ...
    if (nSnsResolution < m_mapResOpts.size()) {
        tSnsResolution = m_mapResOpts[nSnsResolution];
...
AX_BOOL CWebOptionHelper::GetAiInfoStr(AX_U8 nSnsID, AX_CHAR* pOutBuf, AX_U32 nSize) {
    ...
    AX_S32 nCount = snprintf(pOutBuf, nSize,
        ...
        vehicle_roi: {enable:%s, min_width:%d, min_height:%d, mode:%d}, \
        events: {%s}, \
        svc: {%s}}",
        ADAPTER_INT2BOOLSTR(m_mapSns2AiAttr[nSnsID].bEnable),
        GetDetectModelStr(nSnsID).c_str(),
        10,
        szPushStrgy,
        ADAPTER_INT2BOOLSTR(1),
        GetDetectModelStr(nSnsID).c_str(), szDetModelAttr,
        ...
        ADAPTER_INT2BOOLSTR(m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.bEnable),m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nWidth, m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nHeight,m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.eMode,
        szEvent,
        szSvc);
...
std::string CWebOptionHelper::GetDetectModelStr(AX_U8 nSnsID) {
    if (AX_SKEL_PPL_FACE == m_mapSns2AiAttr[nSnsID].eDetectModel) {
        return "facehuman";
    } else if (AX_SKEL_PPL_HVCP == m_mapSns2AiAttr[nSnsID].eDetectModel) {
        return "hvcfp";
    } else {
        return "---";
    }
...
AX_BOOL CWebOptionHelper::ParseWebRequest(WEB_REQUEST_TYPE_E eReqType, const AX_VOID* pJsonReq, vector<WEB_REQ_OPERATION_T>& vecWebOpr) {
    ...
    switch (eReqType) {
        ...
        case E_REQ_TYPE_CAMERA: {
            ...
            // sensor resolution
            {
                AX_U32 nSnsResolution = m_mapSns2CameraSetting[nSnsID].nSnsResolution;
                WEB_SET_INT(nSnsResolution, pJsonCamera, "sns_resolution");

                WEB_OPR_SNS_RESOLUTION_T tSnsResolution;
                if (nSnsResolution != m_mapSns2CameraSetting[nSnsID].nSnsResolution
                    && GetSnsResolution(nSnsID, nSnsResolution, tSnsResolution)) {
                    WEB_REQ_OPERATION_T tOperation;
                    tOperation.nSnsID = nSnsID;
                    tOperation.nChannel = -1;
                    tOperation.eReqType = E_REQ_TYPE_CAMERA;
                    tOperation.SetOperaType(E_WEB_OPERATION_TYPE_SNS_RESOLUTION);
                    tOperation.tSnsResolution = tSnsResolution;

                    WEB_OPR_SNS_RESOLUTION_T tSnsOldResolution;
                    GetSnsResolution(nSnsID, m_mapSns2CameraSetting[nSnsID].nSnsResolution, tSnsOldResolution);

                    vecWebOpr.emplace_back(tOperation);
                    LOG_MM_C(WEB_OPTION, "[%d] camera resolution changed:[%dx%d]=> [%dx%d]",
                                nSnsID, tSnsOldResolution.nSnsWidth, tSnsOldResolution.nSnsHeight, tSnsResolution.nSnsWidth, tSnsResolution.nSnsHeight);

                    m_mapSns2CameraSetting[nSnsID].nSnsResolution = nSnsResolution;
                    ...
                    // change main video resolution
                    {
                        ...
                        tOperation.nSnsID = nSnsID;
                        tOperation.nGroup = 0;
                        tOperation.nChannel = m_mapSns2VideoAttr[nSnsID][0].nUniChn; /* unique channel id corresponding to venc id */
                        tOperation.eReqType = E_REQ_TYPE_VIDEO;
                        tOperation.SetOperaType(E_WEB_OPERATION_TYPE_RESOLUTION);
                        tOperation.tResolution.nWidth = tSnsResolution.nSnsWidth;
                        tOperation.tResolution.nHeight = tSnsResolution.nSnsHeight;
                        tOperation.tResolution.bUpdateIvpsGrp = AX_TRUE;

                        WEB_VIDEO_ATTR_T tAttr = GetVideo(nSnsID, 0);
                        tOperation.tResolution.nEncoderType = tAttr.nEncoderType;

                        vecWebOpr.emplace_back(tOperation);
                        LOG_MM_C(WEB_OPTION, "[%d,%d] changed: [%d, %d] => [%d, %d]", nSnsID, 0,
                                tAttr.nWidth, tAttr.nHeight, tOperation.tResolution.nWidth, tOperation.tResolution.nHeight);

                        m_mapSns2VideoAttr[nSnsID][0].nWidth = tOperation.tResolution.nWidth;
                        m_mapSns2VideoAttr[nSnsID][0].nHeight = tOperation.tResolution.nHeight;
                    }
                }
            }
            ...
            // sensor mode
            {
                ...
                if ((m_mapSns2CameraSetting[nSnsID].bSnsModeEnable
                        && nSnsMode != m_mapSns2CameraSetting[nSnsID].nSnsMode)
            ...
            // sensor rotation
            if (m_mapSns2CameraSetting[nSnsID].bRotationEnable) {
            ...
            // PN mode
            if (m_mapSns2CameraSetting[nSnsID].bPNModeEnable) {
            ...
            // mirror flip
            if (m_mapSns2CameraSetting[nSnsID].bMirrorFlipEnable) {
            ...
            for (AX_U8 i = 0; i < nSize; i++) {
                ...
                if (szResStr) {
                    ...
                    if (ParseResStr(strResStr, nWidth, nHeight) && (nWidth != tOldAttr.nWidth || nHeight != tOldAttr.nHeight)) {
                        ...
                        tOperation.tResolution.bUpdateIvpsGrp = AX_FALSE;
        ...
        case E_REQ_TYPE_AI: {
            ...
            if (pJsonBodyRoi) {
                ...
                tOperation.nChannel = -1;
                tOperation.eReqType = E_REQ_TYPE_AI;
                tOperation.SetOperaType(E_WEB_OPERATION_TYPE_AI_BODY_AE_ROI);
            ...
            if (pJsonVehicleRoi) {
                ...
                tOperation.nChannel = -1;
                tOperation.eReqType = E_REQ_TYPE_AI;
                tOperation.SetOperaType(E_WEB_OPERATION_TYPE_AI_VEHICLE_AE_ROI);

                WEB_SET_BOOL(tOperation.tAiAeRoi.bEnable, pJsonVehicleRoi, "enable");
                WEB_SET_INT(tOperation.tAiAeRoi.nWidth, pJsonVehicleRoi, "min_width");
                WEB_SET_INT(tOperation.tAiAeRoi.nHeight, pJsonVehicleRoi, "min_height");
                AX_S32 nMode = 0;
                WEB_SET_INT(nMode, pJsonVehicleRoi, "mode");
                tOperation.tAiAeRoi.eMode = (AX_APP_ALGO_AE_ROI_MODE_E)nMode;

                if (m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.bEnable != tOperation.tAiAeRoi.bEnable ||
                    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nWidth != tOperation.tAiAeRoi.nWidth ||
                    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nHeight != tOperation.tAiAeRoi.nHeight) {

                    vecWebOpr.emplace_back(tOperation);

                    LOG_MM_C(WEB_OPTION, "[%d] vehicle changed: [%d %d %d] => [%d %d %d]", nSnsID,
                            m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.bEnable ,
                            m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nWidth,
                            m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nHeight,
                            tOperation.tAiAeRoi.bEnable,
                            tOperation.tAiAeRoi.nWidth,
                            tOperation.tAiAeRoi.nHeight);

                    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.bEnable = tOperation.tAiAeRoi.bEnable;
                    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nWidth = tOperation.tAiAeRoi.nWidth;
                    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.nHeight = tOperation.tAiAeRoi.nHeight;
                    m_mapSns2AiAttr[nSnsID].tAeRoiVehicle.eMode = tOperation.tAiAeRoi.eMode;
                }
            }
...
[%d %d] bg[%d %d] body[%d %d %d] vehicle[%d %d %d] cycle[%d %d %d] face[%d %d %d] plate[%d %d %d]", nSnsID,
        ...
        case E_REQ_TYPE_EZOOM: {
            ...
            WEB_SET_FLOAT(fEZoomRatio, pJsonEzoom, "ezoom_ratio");
            ...
            tOperation.tEZoom.fEZoomRatio = fEZoomRatio;
