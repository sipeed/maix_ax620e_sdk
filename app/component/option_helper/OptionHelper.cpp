...
AX_BOOL COptionHelper::GetCmmPartitions(CMM_PARTITIONS_INFO_T &info) {
#ifndef _OPAL_LIB_
    std::map<std::string, std::string> mapKeys;
    m_iniWrapper.GetAllKeys("cmm", mapKeys);
    ...
    for (auto& v : mapKeys) {
        ...
        std::string strValue = v.second;

        if(strKey.compare("enable") == 0) {
            info.bEnable = strValue.compare("0") == 0 ? AX_FALSE : AX_TRUE;
        } else if (strKey.find("partition") == 0 && strValue != "") {
            std::string strs = strValue + ",";
            ...
            while (pos != strs.npos)
            {
                ...
                {
                case 0:
                    strcpy(info.partitions[info.nCount].szName, temp.c_str());
                    break;
                case 1:
                    info.partitions[info.nCount].nModId = atoi(temp.c_str());
                    break;
                case 2:
                    info.partitions[info.nCount].nGrpId = atoi(temp.c_str());
                    break;
                case 3:
                    info.partitions[info.nCount].nChnId = atoi(temp.c_str());
                    break;
                case 4:
                    info.partitions[info.nCount].nType = atoi(temp.c_str());
                    info.nCount ++;
                    break;
                default:
                    break;
...
AX_U32 COptionHelper::GetDiscardYuvFrmNum() {
#ifndef _OPAL_LIB_
    AX_U32 value = (AX_U32)m_iniWrapper.GetIntValue("sns", "DiscardYuvFrameNum", 0);
    return value;
#else
    return 0;
#endif
