...
#define AUDIO_AI_STEREO_AAC_PERIOD_SIZE (1280) // stereo aac + vqe: 1280; stereo aac: 1024
#define AUDIO_AO_STEREO_AAC_PERIOD_SIZE (1280) // stereo aac + vqe: 1280; stereo aac: 1024
...
static CAudioAed *g_pAedInstance = nullptr;
static AX_BOOL g_bAedStart = AX_FALSE;

static std::unique_ptr<IObserver> g_ACapRawObs;
static std::vector<std::unique_ptr<IObserver>> g_vecACapEncObs;
static std::unique_ptr<IObserver> g_AedObs;

#define DECLARE_AUDIOWRAPPER_THREAD_SAFE_API() std::lock_guard<std::mutex> lck(audioMutex)

static AX_S32 audio_capture_attr_parser(const AX_APP_AUDIO_ATTR_PTR pstAttr, AUDIO_CAP_DEV_ATTR_T &stACapDevAttr, AX_U32 &nACapPipeNum,
    ...
    if (pstAttr->stCapAttr.bEnable) {
        AX_U32 nChnCnt = 2;
        AX_U32 nPeriodSize = pstAttr->stDevCommAttr.nPeriodSize;
        ...
        for (AX_U32 i = 0; i < AX_APP_AUDIO_CHAN_BUTT; i++) {
            if (pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].bEnable) {
                ...
                if (AX_APP_AUDIO_SOUND_MODE_MONO == pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].eSoundMode) {
                    ...
                    if (PT_AAC == pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].eType) {
                        nPeriodSize = AUDIO_AI_STEREO_AAC_PERIOD_SIZE;
                    }
                }
                ...
                if (PT_AAC == stACapPipeAttr[nACapPipeNum].eType) {
                    stACapPipeAttr[nACapPipeNum].stEncoderAttr.stAacEncoder.eAacType =
                        _AX_AAC_TYPE(pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].stAacEncoder.eAacType);
                    stACapPipeAttr[nACapPipeNum].stEncoderAttr.stAacEncoder.eTransType =
                        _AX_AAC_TRANS_TYPE(pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].stAacEncoder.eTransType);
                } else {
                    stACapPipeAttr[nACapPipeNum].stEncoderAttr.stDefEncoder.nBitRate =
                        pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].nBitRate;
                    stACapPipeAttr[nACapPipeNum].stEncoderAttr.stDefEncoder.eBitWidth = _AX_BIT_WIDTH(pstAttr->stDevCommAttr.eBitWidth);
                    stACapPipeAttr[nACapPipeNum].stEncoderAttr.stDefEncoder.eSampleRate =
                        _AX_SAMPLE_RATE(pstAttr->stDevCommAttr.eSampleRate);
                }
        ...
        // capture dev
        {
            ...
            stACapDevAttr.nFSChannels = pstAttr->stCapAttr.stDevAttr.nFSChannels;
            stACapDevAttr.eLayoutMode = _AX_LAYOUT_MODE(pstAttr->stCapAttr.stDevAttr.eLayoutMode);
            stACapDevAttr.nPeriodSize = nPeriodSize;
            ...
            if (AX_APP_AUDIO_AEC_FIXED == pstAttr->stCapAttr.stDevAttr.stVqeAttr.stAecAttr.eType) {
                stACapDevAttr.stVqeAttr.stAecCfg.stAecFixedCfg.eRoutingMode =
                    _AX_ROUTING_MODE(pstAttr->stCapAttr.stDevAttr.stVqeAttr.stAecAttr.stFixedAttr.eMode);
            } else if (AX_APP_AUDIO_AEC_FLOAT == pstAttr->stCapAttr.stDevAttr.stVqeAttr.stAecAttr.eType) {
                stACapDevAttr.stVqeAttr.stAecCfg.stAecFloatCfg.enSuppressionLevel =
                    _AX_SUPPRESSION_LEVEL(pstAttr->stCapAttr.stDevAttr.stVqeAttr.stAecAttr.stFloatAttr.eLevel);
            }

            // ns
            stACapDevAttr.stVqeAttr.stNsCfg.bNsEnable = (AX_BOOL)pstAttr->stCapAttr.stDevAttr.stVqeAttr.stAnsAttr.bEnable;
            stACapDevAttr.stVqeAttr.stNsCfg.enAggressivenessLevel =
                _AX_AGGRESSIVENESS_LEVEL(pstAttr->stCapAttr.stDevAttr.stVqeAttr.stAnsAttr.eLevel);
            ...
            // vad
            stACapDevAttr.stVqeAttr.stVadCfg.bVadEnable = (AX_BOOL)pstAttr->stCapAttr.stDevAttr.stVqeAttr.stVadAttr.bEnable;
            stACapDevAttr.stVqeAttr.stVadCfg.u32VadLevel = pstAttr->stCapAttr.stDevAttr.stVqeAttr.stVadAttr.nVadLevel;
        }
    }
...
static AX_S32 audio_play_attr_parser(const AX_APP_AUDIO_ATTR_PTR pstAttr, AUDIO_PLAY_DEV_ATTR_T &stAPlayDevAttr, AX_U32 &nAPlayPipeNum,
    ...
    if (pstAttr->stPlayAttr.bEnable) {
        ...
        for (AX_U32 i = 0; i < AX_APP_AUDIO_CHAN_BUTT; i++) {
            if (pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].bEnable) {
                ...
                } else if (PT_G711A == stAPlayPipeAttr[nAPlayPipeNum].eType || PT_G711U == stAPlayPipeAttr[nAPlayPipeNum].eType) {
                    stAPlayPipeAttr[nAPlayPipeNum].nBlkSize = ADEC_G711_BLK_SIZE;
                } else {
                ...
                if (AX_APP_AUDIO_SOUND_MODE_MONO == pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].eSoundMode) {
                    ...
                    if (PT_AAC == pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].eType) {
                        nPeriodSize = AUDIO_AO_STEREO_AAC_PERIOD_SIZE;
                    }
                }

                if (PT_AAC == stAPlayPipeAttr[nAPlayPipeNum].eType) {
                    stAPlayPipeAttr[nAPlayPipeNum].stDecoderAttr.stAacDecoder.eTransType =
                        _AX_AAC_TRANS_TYPE(pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].stAacDecoder.eTransType);
                } else {
                    stAPlayPipeAttr[nAPlayPipeNum].stDecoderAttr.stDefDecoder.nBitRate =
                        pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].nBitRate;
                    stAPlayPipeAttr[nAPlayPipeNum].stDecoderAttr.stDefDecoder.eBitWidth = _AX_BIT_WIDTH(pstAttr->stDevCommAttr.eBitWidth);
                    stAPlayPipeAttr[nAPlayPipeNum].stDecoderAttr.stDefDecoder.eSampleRate =
                        _AX_SAMPLE_RATE(pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].eSampleRate);
                }
        ...
        // play dev
        {
            ...
            stAPlayDevAttr.nPeriodSize = nPeriodSize;
            ...
            stAPlayDevAttr.stVqeAttr.stNsCfg.enAggressivenessLevel =
                _AX_AGGRESSIVENESS_LEVEL(pstAttr->stPlayAttr.stDevAttr.stVqeAttr.stAnsAttr.eLevel);
...
static AX_S32 audio_aed_attr_parser(const AX_APP_AUDIO_ATTR_PTR pstAttr, AUDIO_AED_DEV_ATTR_T &stAedDevAttr, AED_ALGO_CONFIG_T& stAedAlgoConfig) {
    // algo param
    AX_APP_ALGO_AUDIO_PARAM_T stAlgoParam = ALGO_AUDIO_PARAM();

    // aed device
    stAedDevAttr.bEnable = stAlgoParam.bEnable;
    stAedDevAttr.nCardId = pstAttr->stCapAttr.stDevAttr.nCardId;
    stAedDevAttr.nDeviceId = pstAttr->stCapAttr.stDevAttr.nDeviceId;

    // algo config
    stAedAlgoConfig.nCardId = pstAttr->stCapAttr.stDevAttr.nCardId;
    stAedAlgoConfig.nDeviceId = pstAttr->stCapAttr.stDevAttr.nDeviceId;

    // param
    stAedAlgoConfig.stAlgoAttr.bEnable = stAlgoParam.bEnable;
    stAedAlgoConfig.stAlgoAttr.fThreshold = stAlgoParam.fThreshold;
    stAedAlgoConfig.stAlgoAttr.nInterval = stAlgoParam.nInterval;
...
static AX_S32 audio_aed_start(AX_VOID) {
    if (!g_bAedStart) {
        AX_BOOL bRet = AX_TRUE;

        if (g_pAedInstance) {
            bRet = g_pAedInstance->Start();
        }
...
static AX_S32 audio_aed_stop(AX_VOID) {
    if (g_bAedStart) {
        AX_BOOL bRet = AX_TRUE;

        if (g_pAedInstance) {
            bRet = g_pAedInstance->Stop();
        }
        ...
        g_bAedStart = AX_FALSE;
    }

    return 0;
}

static AX_S32 audio_aed_deinit(AX_VOID) {
    if (g_pAedInstance) {
        audio_aed_stop();

        g_pAedInstance->DeInit();
        delete g_pAedInstance;
        g_pAedInstance = nullptr;
    }

    return 0;
}

static AX_S32 audio_aed_init(const AX_APP_AUDIO_ATTR_PTR pstAttr) {
    AX_S32 nRet = 0;

    // audio aed (audio capture enable will always open aed instance)
    if (pstAttr->stCapAttr.bEnable) {
        AUDIO_AED_DEV_ATTR_T stAedDevAttr;
        AED_ALGO_CONFIG_T stAedAlgoConfig;

        audio_aed_attr_parser(pstAttr, stAedDevAttr, stAedAlgoConfig);

        g_pAedInstance = new CAudioAed();
        ...
        if (!g_pAedInstance->Init(&stAedDevAttr, &stAedAlgoConfig)) {
            LOG_M_E(AUDIO_WRAPPER, "Start audio aed failed.");
            nRet = -1;
            goto EXIT;
        }
    }
...
EXIT:
    if (0 != nRet) {
        pstAttr->stCapAttr.bEnable = AX_FALSE;
        pstAttr->stPlayAttr.bEnable = AX_FALSE;
        SET_APP_AUDIO_ATTR(*pstAttr);
    }
...
AX_S32 AX_APP_Audio_Deinit(AX_VOID) {
    LOG_MM_C(AUDIO_WRAPPER, "+++");

    DECLARE_AUDIOWRAPPER_THREAD_SAFE_API();

    AX_S32 nRet = audio_aed_deinit();

    if (0 != nRet) {
        goto EXIT;
    }

    nRet = audio_capture_deinit();
...
AX_S32 AX_APP_Audio_Start(AX_VOID) {
    ...
    nRet = audio_aed_start();

EXIT:
    if (0 != nRet) {
        AX_APP_AUDIO_ATTR_T stAttr = APP_AUDIO_ATTR();
        stAttr.stCapAttr.bEnable = AX_FALSE;
        stAttr.stPlayAttr.bEnable = AX_FALSE;
        SET_APP_AUDIO_ATTR(stAttr);
    }
...
AX_S32 AX_APP_Audio_Stop(AX_VOID) {
    ...
    AX_S32 nRet = audio_aed_stop();

    if (0 != nRet) {
        goto EXIT;
    }

    nRet = audio_capture_stop();
...
AX_S32 AX_APP_Audio_SetAttr(const AX_APP_AUDIO_ATTR_PTR pstAttr) {
    ...
    if (pstAttr) {
        ...
        if (pstAttr->stCapAttr.bEnable != stAudioAttr.stCapAttr.bEnable) {
            ...
            for (AX_U32 i = 0; i < AX_APP_AUDIO_CHAN_BUTT; i++) {
                if (pstAttr->stCapAttr.stPipeAttr.stAudioChanAttr[i].bEnable !=
                    stAudioAttr.stCapAttr.stPipeAttr.stAudioChanAttr[i].bEnable) {
                    bReContruct = AX_TRUE;
                    break;
                }
            }
        ...
        if (pstAttr->stPlayAttr.bEnable != stAudioAttr.stPlayAttr.bEnable) {
            ...
            for (AX_U32 i = 0; i < AX_APP_AUDIO_CHAN_BUTT; i++) {
                if (pstAttr->stPlayAttr.stPipeAttr.stAudioChanAttr[i].bEnable !=
                    stAudioAttr.stPlayAttr.stPipeAttr.stAudioChanAttr[i].bEnable) {
                    bReContruct = AX_TRUE;
                    break;
                }
            }
        ...
        if (bReContruct) {
            ...
            // aed dev
            g_pAedInstance->SetDevAttr(stAedDevAttr);

            // aed algo
            g_pAedInstance->SetAlgoAttr(stAedAlgoConfig);
        }
...
AX_S32 AX_APP_Audio_GetAedAttr(AX_APP_ALGO_AUDIO_PARAM_PTR pstAttr) {
    DECLARE_AUDIOWRAPPER_THREAD_SAFE_API();

    if (pstAttr) {
        *pstAttr = ALGO_AUDIO_PARAM();
    }

    return 0;
}

AX_S32 AX_APP_Audio_SetAedAttr(const AX_APP_ALGO_AUDIO_PARAM_PTR pstAttr) {
    AX_S32 nRet = 0;
    ...
    if (pstAttr) {
        ...
        // audio aed
        AX_BOOL bReContruct = AX_FALSE;
        if (stAudioAttr.stCapAttr.bEnable
            && !g_pAedInstance) {
            bReContruct = AX_TRUE;
        }

        SET_ALGO_AUDIO_PARAM(*pstAttr);

        if (bReContruct) {
            audio_aed_deinit();

            nRet = audio_aed_init(&stAudioAttr);
            ...
            audio_aed_attr_parser(&stAudioAttr, stAedDevAttr, stAedAlgoConfig);
    ...
    if (g_pAPlayInstance) {
        ...
        if (g_pAPlayInstance->GetAdecAttr((AX_U32)eChan, stAttr)) {
            if (stAttr.eType != eType) {
                ...
                } else if (eType == PT_G711A || eType == PT_G711U) {
                    stAttr.nBlkSize = ADEC_G711_BLK_SIZE;
                } else {
...
AX_S32 AX_APP_Audio_PlayFile(AX_APP_AUDIO_CHAN_E eChan, AX_PAYLOAD_TYPE_E eType, const AX_CHAR *pstrFileName, AX_S32 nLoop,
                             AX_APP_AUDIO_PLAYFILERESULT_CALLBACK callback, AX_VOID *pUserData) {
    DECLARE_AUDIOWRAPPER_THREAD_SAFE_API();
...
AX_S32 AX_APP_Audio_GetEncoderAttr(AX_APP_AUDIO_CHAN_E eChan, AX_APP_AUDIO_ENCODER_ATTR_PTR pstAttr) {
    ...
    if (eChan < AX_APP_AUDIO_CHAN_BUTT && pstAttr && g_pACapInstance) {
...
AX_S32 AX_APP_Audio_GetPlayPipeAttr(AX_APP_AUDIO_CHAN_E eChan, AX_APP_AUDIO_PLAY_CHAN_ATTR_PTR pstAttr) {
    ...
    if (eChan < AX_APP_AUDIO_CHAN_BUTT && pstAttr && g_pAPlayInstance) {
        AX_APP_AUDIO_ATTR_T stAudioAttr = APP_AUDIO_ATTR();
...
AX_S32 AX_APP_Audio_GetAacEncoderConfigBuf(AX_APP_AUDIO_CHAN_E eChan, const AX_U8 **ppConfBuf, AX_U32 *pDataSize) {
    ...
    if (eChan < AX_APP_AUDIO_CHAN_BUTT && ppConfBuf && g_pACapInstance) {
...
AX_S32 AX_APP_Audio_SetAacDecoderConfigBuf(AX_APP_AUDIO_CHAN_E eChan, const AX_U8 *pConfBuf, AX_U32 nDataSize) {
    ...
    if (eChan < AX_APP_AUDIO_CHAN_BUTT && pConfBuf && g_pAPlayInstance) {
        // TODO
        return -1;
    }
...
AX_S32 AX_APP_Audio_RegisterDetectResultCallback(const AX_APP_AUDIO_DETECTRESULT_CALLBACK callback, AX_VOID *pUserData) {
    ...
    if (g_pAedInstance) {
        g_AedObs = CObserverMaker::CreateObserver<CAudioWrapperObserver>((AX_VOID *)callback, (AX_VOID *)pUserData);
        g_pAedInstance->RegObserver(g_AedObs.get());
...
AX_S32 AX_APP_Audio_UnRegisterDetectResultCallback(AX_VOID) {
    if (g_pAedInstance) {
        g_pAedInstance->UnregObserver(g_AedObs.get());
...
AX_S32 AX_APP_Audio_RegDetectResultObserver(AX_VOID *pObserver) {
    DECLARE_AUDIOWRAPPER_THREAD_SAFE_API();

    if (g_pAedInstance) {
        g_pAedInstance->RegObserver((IObserver *)pObserver);
...
AX_S32 AX_APP_Audio_UnregDetectResultObserver(AX_VOID *pObserver) {
    DECLARE_AUDIOWRAPPER_THREAD_SAFE_API();

    if (g_pAedInstance) {
        g_pAedInstance->UnregObserver((IObserver *)pObserver);
