...
static AX_BOOL IsUpTalkVqeEnabled(const AX_AP_UPTALKVQE_ATTR_T &stVqeAttr) {
    return (AX_BOOL)((stVqeAttr.stAecCfg.enAecMode != AX_AEC_MODE_DISABLE) || (stVqeAttr.stNsCfg.bNsEnable != AX_FALSE) ||
                     (stVqeAttr.stAgcCfg.bAgcEnable != AX_FALSE) || (stVqeAttr.stVadCfg.bVadEnable != AX_FALSE));
}
}  // namespace

CAudioCapDev::CAudioCapDev(const AUDIO_CAP_DEV_ATTR_T &stAttr) : m_stCapDevAttr(stAttr) {
...
AX_BOOL CAudioCapDev::Init() {
    if (AX_INVALID_POOLID == m_nACapDevPoolId) {
        ...
        if (m_stCapDevAttr.eLayoutMode == AX_AI_MIC_MIC) {
            stPoolConfig.BlkSize = AUDIO_CAP_DEV_DEFAULT_STEREO_BLK_SIZE;
        } else {
...
AX_BOOL CAudioCapDev::Restart(const AUDIO_CAP_DEV_ATTR_T &stAttr) {
    // TODO:
    if (stAttr.nBlkSize != m_stCapDevAttr.nBlkSize || stAttr.nBlkCnt != m_stCapDevAttr.nBlkCnt) {
        Stop();
...
AX_BOOL CAudioCapDev::Create() {
    ...
    stAttr.u32PeriodCount = AUDIO_CAP_DEV_PERIOD_COUNT;
    stAttr.u32ChnCnt = m_stCapDevAttr.nFSChannels;
...
AX_BOOL CAudioCapDev::SetAttr(const AUDIO_CAP_DEV_ATTR_T &stAttr) {
    if (m_bStart) {
        if (stAttr.bLink != m_stCapDevAttr.bLink || stAttr.nCardId != m_stCapDevAttr.nCardId ||
            stAttr.nDeviceId != m_stCapDevAttr.nDeviceId || stAttr.nBlkSize != m_stCapDevAttr.nBlkSize ||
            stAttr.nBlkCnt != m_stCapDevAttr.nBlkCnt || stAttr.nDepth != m_stCapDevAttr.nDepth ||
            stAttr.nChnCnt != m_stCapDevAttr.nChnCnt || stAttr.nPeriodSize != m_stCapDevAttr.nPeriodSize ||
            stAttr.eBitWidth != m_stCapDevAttr.eBitWidth || stAttr.eSampleRate != m_stCapDevAttr.eSampleRate ||
...
AX_BOOL CAudioCapDev::SetResample(const AX_AUDIO_SAMPLE_RATE_E &eSampleRate) {
    ...
    if (eSampleRate != m_stCapDevAttr.eSampleRate && !m_stResampleAttr.bEnable) {
        m_stResampleAttr.bEnable = AX_TRUE;
    ...
    else if (eSampleRate == m_stCapDevAttr.eSampleRate && m_stResampleAttr.bEnable) {
        m_stResampleAttr.bEnable = AX_FALSE;
        m_stResampleAttr.eSampleRate = eSampleRate;
        m_eAencSampleRate = eSampleRate;
