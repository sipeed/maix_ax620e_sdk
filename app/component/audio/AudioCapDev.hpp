...
#define AUDIO_CAP_DEV_DEFAULT_STEREO_BLK_SIZE (12800)
...
typedef struct audioCAP_DEV_ATTR_T {
    ...
    audioCAP_DEV_ATTR_T() {
        ...
        stVqeAttr = {.s32SampleRate = eSampleRate,
                     .u32FrameSamples = nPeriodSize,
                     .msInSndCardBuf = 0,
                     .stAecCfg = {.enAecMode = AX_AEC_MODE_FLOAT, {.stAecFloatCfg = {.enSuppressionLevel = AX_SUPPRESSION_LEVEL_HIGH}}},
                     .stNsCfg = {.bNsEnable = AX_TRUE, .enAggressivenessLevel = AX_AGGRESSIVENESS_LEVEL_HIGH},
                     .stAgcCfg = {.bAgcEnable = AX_TRUE, .enAgcMode = AX_AGC_MODE_FIXED_DIGITAL, .s16TargetLevel = -3, .s16Gain = 9},
                     .stVadCfg = {.bVadEnable = AX_FALSE, .u32VadLevel = 2}};
    }
...
public:
    ...
    AX_BOOL GetAttr(AUDIO_CAP_DEV_ATTR_T &stAttr);
    AX_BOOL SetAttr(const AUDIO_CAP_DEV_ATTR_T &stAttr);

    AX_BOOL SetResample(const AX_AUDIO_SAMPLE_RATE_E &eSampleRate);
    ...
    AX_U32 GetEncoderPeriodSize(AX_VOID) {
        return m_stCapDevAttr.nPeriodSize;
    }
