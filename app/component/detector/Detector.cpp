...
static AX_VOID SkelResult2DetectResult(DETECT_RESULT_T &hvcfp, const AX_SKEL_RESULT_T *pstResult, CDetector *pThis) {
    ...
    for (AX_U32 i = 0; i < pstResult->nObjectSize; ++i) {
        ...
        stItem.stAttr.stFacePoseBlur.fPitch = stObjectItem.stFaceAttr.stPoseBlur.fPitch;
        stItem.stAttr.stFacePoseBlur.fYaw = stObjectItem.stFaceAttr.stPoseBlur.fYaw;
        stItem.stAttr.stFacePoseBlur.fRoll = stObjectItem.stFaceAttr.stPoseBlur.fRoll;
        stItem.stAttr.stFacePoseBlur.fBlur = stObjectItem.stFaceAttr.stPoseBlur.fBlur;
...
static AX_VOID SkelResultCallback(AX_SKEL_HANDLE pHandle, AX_SKEL_RESULT_T *pstResult, AX_VOID *pthis) {
    ...
    pThis->SetSnsDetectResult(hvcfp);
...
AX_VOID CDetector::RunDetect(AX_VOID *pArg) {
    ...
    while (m_DetectThread.IsRunning()) {
        pAxFrame = nullptr;
        for (nFrameQIndex = nNextIndex; nFrameQIndex < nGrpCount; ++nFrameQIndex) {
            if (m_arrFrameQ[nFrameQIndex].Pop(pAxFrame, 0)) {
                break;
            }
        }

        if (!pAxFrame || nFrameQIndex >= nGrpCount) {
            nNextIndex = 0;
            CElapsedTimer::GetInstance()->mSleep(10);
            continue;
        } else {
            nNextIndex = nFrameQIndex + 1;
            if (nNextIndex >= nGrpCount) {
                nNextIndex = 0;
            }
        }
        ...
        if (!pPrivData) {
            ...
            pPrivData->nSnsId = m_stAttr.nSnsId[nFrameQIndex];
        }
        skelFrame.nFrameId = ++nFrameId;
        skelFrame.nStreamId = m_stAttr.nSnsId[nFrameQIndex];
        ...
        if (AX_SKEL_SUCC != ret) {
            ...
            if (AX_ERR_SKEL_QUEUE_FULL != ret
                ...
                LOG_MM_E(DETECTOR,
                         ...
                         "stride %d blkId "
                         "0x%x, size:%u",
                         ret, m_stAttr.nSnsId[nFrameQIndex], pAxFrame->nGrp, pAxFrame->nChn, pAxFrame->stFrame.stVFrame.stVFrame.u64SeqNum,
...
AX_BOOL CDetector::Init(const DETECTOR_ATTR_T &stAttr) {
    ...
    AX_BOOL res = AX_TRUE;
    m_arrFrameQ = new (std::nothrow) CAXLockQ<CAXFrame *>[m_stAttr.nGrpCount];
    if (!m_arrFrameQ) {
    ...
    LOG_MM_I(DETECTOR, "m_stAttr.width:%d, height:%d, nGrpCount: %d, modelPath:%s",
                        m_stAttr.nWidth, m_stAttr.nHeight, m_stAttr.nGrpCount, m_stAttr.strModelPath.c_str());
    /* [1]: SKEL init */
    ...
    do {
        ...
        if (AX_SKEL_SUCC != ret) {
            ...
            if (!bHVCFP) {
                LOG_M_E(DETECTOR, "%s: SKEL not found related models", __func__);
                res = AX_FALSE;
                break;
            }
        }
        ...
        for (AX_U8 i = 0; i < m_stAttr.nGrpCount && i < MAX_DETECTOR_GROUP_NUM; i++) {
            SetConfig(m_stAttr.nSnsId[i], ALGO_HVCFP_PARAM(m_stAttr.nSnsId[i]));
        }
...
AX_BOOL CDetector::SendFrame(AX_U8 nSnsId, CAXFrame *paxFrame) {
    if (!m_initState) {
        paxFrame->FreeMem();
        return AX_TRUE;
    } else {
        if (!m_mapSnsAiEnable[nSnsId]) {
            paxFrame->FreeMem();
            return AX_TRUE;
        }

        AX_U8 i = 0;
        for (; i < m_stAttr.nGrpCount; i++) {
            if ((AX_U8)m_stAttr.nSnsId[i] == nSnsId) {
                break;
            }
        }

        if (i < m_stAttr.nGrpCount) {
            if (!m_arrFrameQ[i].Push(paxFrame)) {
                paxFrame->FreeMem();
                CElapsedTimer::GetInstance()->mSleep(1);
            }
        } else {
            paxFrame->FreeMem();
            LOG_MM_E(DETECTOR, "Invalid sensor id: %d.", nSnsId);
            return AX_TRUE;
        }
    }
    return AX_TRUE;
...
AX_VOID CDetector::ClearQueue(AX_S32 nIndex) {
    AX_U32 nCount = m_arrFrameQ[nIndex].GetCount();
    if (nCount > 0) {
        CAXFrame *pAxFrame;
        for (AX_U32 i = 0; i < nCount; ++i) {
            if (m_arrFrameQ[nIndex].Pop(pAxFrame, 0)) {
                pAxFrame->FreeMem();
            }
        }
    }
...
AX_VOID CDetector::Enable(AX_U8 nSnsId, AX_BOOL bEnable) {
    m_mapSnsAiEnable[nSnsId] = bEnable;
    if (!bEnable) {
        DETECT_RESULT_T hvcfp;
        hvcfp.nBodyCount = 0;
...
AX_VOID CDetector::GetAeRoi(AX_U32 nSnsId, AX_U32 nItemSize,
    ...
    if (m_pSensorMgr) {
        ...
        if (pSensorInstance) {
            pSensorInstance->GetResolution(nImageWidth, nImageHeight);
        }
    }
...
AX_BOOL CDetector::SetAeRoiAttr(AX_U32 nSnsId, const AX_APP_ALGO_AE_ROI_CONFIG_T stAeRoiConfig[AX_APP_ALGO_HVCFP_TYPE_BUTT], AX_BOOL bUpdateParam) {
    if (m_pSensorMgr) {
        AX_BOOL bFaceAeRoiEnable = (AX_BOOL)(stAeRoiConfig[AX_APP_ALGO_HVCFP_BODY].bEnable
                                            || stAeRoiConfig[AX_APP_ALGO_HVCFP_FACE].bEnable);

        AX_BOOL bVehicleAeRoiEnable = (AX_BOOL)(stAeRoiConfig[AX_APP_ALGO_HVCFP_VEHICLE].bEnable
                                            || stAeRoiConfig[AX_APP_ALGO_HVCFP_CYCLE].bEnable
                                            || stAeRoiConfig[AX_APP_ALGO_HVCFP_PLATE].bEnable);

        AX_BOOL bAeRoiEnable = (AX_BOOL)(bFaceAeRoiEnable || bVehicleAeRoiEnable);

        if (!bAeRoiEnable) {
            RestoreAeRoi(nSnsId);
        }

        m_pSensorMgr->UpdateAeRoi(nSnsId, bFaceAeRoiEnable, bVehicleAeRoiEnable);
    }
...
AX_VOID CDetector::SetSnsDetectResult(const DETECT_RESULT_T& tDetectResult) {
    if (m_pSensorMgr) {
        COORDINATION_SKEL_RESULT_T tSkelResult;
        if (tDetectResult.nBodyCount > 0) {
            for (AX_U8 i = 0; i < tDetectResult.nBodyCount; i++) {
                if (tDetectResult.stBodys[i].eTrackStatus == AX_APP_ALGO_TRACK_STATUS_NEW
                    || tDetectResult.stBodys[i].eTrackStatus == AX_APP_ALGO_TRACK_STATUS_UPDATE) {
                    tSkelResult.bBodyFound = AX_TRUE;
                    break;
                }
            }
        }

        if (tDetectResult.nVehicleCount > 0) {
            for (AX_U8 i = 0; i < tDetectResult.nVehicleCount; i++) {
                if (tDetectResult.stVehicles[i].eTrackStatus == AX_APP_ALGO_TRACK_STATUS_NEW
                    || tDetectResult.stVehicles[i].eTrackStatus == AX_APP_ALGO_TRACK_STATUS_UPDATE) {
                    tSkelResult.bVehicleFound = AX_TRUE;
                    break;
                }
            }
        }

        if (tDetectResult.nCycleCount > 0) {
            for (AX_U8 i = 0; i < tDetectResult.nCycleCount; i++) {
                if (tDetectResult.stCycles[i].eTrackStatus == AX_APP_ALGO_TRACK_STATUS_NEW
                    || tDetectResult.stCycles[i].eTrackStatus == AX_APP_ALGO_TRACK_STATUS_UPDATE) {
                    tSkelResult.bCycleFound = AX_TRUE;
                    break;
                }
            }
        }

        m_pSensorMgr->SetSkelDetectResult(tDetectResult.nSnsId, tDetectResult.nSeqNum, tSkelResult);
    }
}
