...
AX_VOID CAttrParser::FaceAttrResult(const AX_SKEL_OBJECT_ITEM_T *pstObjectItems, ATTR_INFO_T *pAttrInfo) {
    ...
    pAttrInfo->bExist = pstObjectItems->bFaceAttr;

    if (pAttrInfo->bExist) {
        pAttrInfo->stFaceInfo.nAge = pstObjectItems->stFaceAttr.nAge;
        pAttrInfo->stFaceInfo.nGender = pstObjectItems->stFaceAttr.nGender;

        if (pstObjectItems->stFaceAttr.nMask == 0) {
            pAttrInfo->stFaceInfo.eRespirator = AX_APP_ALGO_FACE_RESPIRATOR_NONE;
        } else {
            pAttrInfo->stFaceInfo.eRespirator = AX_APP_ALGO_FACE_RESPIRATOR_COMMON;
        }
    }
}
...
AX_VOID CAttrParser::PlateAttrResult(const AX_SKEL_OBJECT_ITEM_T *pstObjectItems, ATTR_INFO_T *pAttrInfo) {
    ...
    for (size_t i = 0; i < pstObjectItems->nMetaInfoSize; i++) {
        if (!strcmp(pstObjectItems->pstMetaInfo[i].pstrType, "plate_attr")) {
            ...
            strncpy(pAttrInfo->stPlateInfo.szNum, (const AX_CHAR *)plat_attr.strPlateCode.c_str(),
                    sizeof(pAttrInfo->stPlateInfo.szNum) - 1);
        }
    }
}
...
AX_VOID CAttrParser::VehicleAttrResult(const AX_SKEL_OBJECT_ITEM_T *pstObjectItems, ATTR_INFO_T *pAttrInfo) {
    ...
    for (size_t i = 0; i < pstObjectItems->nMetaInfoSize; i++) {
        if (!strcmp(pstObjectItems->pstMetaInfo[i].pstrType, "plate_attr")) {
            ...
            strncpy(pAttrInfo->stVehicleInfo.stPlateInfo.szNum, (const AX_CHAR *)plat_attr.strPlateCode.c_str(),
                    sizeof(pAttrInfo->stVehicleInfo.stPlateInfo.szNum) - 1);
        }
    }
}
