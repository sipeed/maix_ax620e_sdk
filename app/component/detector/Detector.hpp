...
#define MAX_DETECTOR_GROUP_NUM (3)
#define MAX_DETECTOR_PRIVATE_DATA_NUM (8)

typedef struct _DETECTOR_ATTR_T {
    AX_U8 nGrpCount{0};
    AX_S32 nGrp{0};
    ...
    AX_U32 nHeight{0};
    AX_F32 fFramerate{0};
    AX_S8 nSnsId[MAX_DETECTOR_GROUP_NUM]{-1, -1, -1};
} DETECTOR_ATTR_T;
...
public:
    ...
    AX_BOOL SendFrame(AX_U8 nSnsId, CAXFrame* axFrame);
    ...
    AX_VOID SetSnsDetectResult(const DETECT_RESULT_T& tDetectResult);

protected:
    AX_VOID RunDetect(AX_VOID* pArg);
    AX_VOID ClearQueue(AX_S32 nIndex);
