  ...
  <div class="total_container">
    <el-container class="preview_outside_container" >
      <el-container :class="mode_option == 'ai' ? ( snapshotEnable && flashEnable ? 'preview_main_container_ai_2cap' : !snapshotEnable && !flashEnable ? 'preview_main_container_ai' : 'preview_main_container_ai_1cap') : (snapshotEnable && flashEnable ? 'preview_main_container_normal_2cap' : !snapshotEnable && !flashEnable ? 'preview_main_container_normal' : 'preview_main_container_normal_1cap')">
        <el-main class="preview_capture_left_container">
          <el-container id="video_container_0" class="preview_container">
            <el-main ref="axMain0" class="video_container">
              <video id="myVideo0" v-show="streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].showVideoStatus" muted="isMuted" ref="axVideoRef0" class="axVideo" autoplay
                playsinline oncontextmenu="return false;">
              </video>
              <canvas id="myMjpeg0" v-show="!streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].showVideoStatus" class="axVideo2">
              </canvas>
            </el-main>
              ...
              <div class="left_controls_container">
                <div class="size_option">
                  <el-dropdown @command="changeSizeOption0" size="mini" placement="top-start" trigger="click">
                    <el-button size="mini" class="el-dropdown-link" type="text">{{ sizeOptions[streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].sizeOptionInd].label
                    }}</el-button>
                ...
                <div>
                  <el-dropdown @command="changeStream0" size="mini" placement="top" trigger="click">
                    <el-button size="mini" class="el-dropdown-link" type="text">{{
                      streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].videoInfo[streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].streamInd].label
                    }}</el-button>
                    <el-dropdown-menu slot="dropdown" :-to-body="false">
                      <el-dropdown-item :command="item.id + ''" :icon="item.icon" v-for="item in streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].videoInfo"
                        :key="item.id">{{ item.label }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </div>
                <div class="src_option" v-show="sns_num >= 3">
                  <el-dropdown @command="changeSnsOptLeft" size="mini" placement="top" trigger="click">
                    <el-button size="mini" class="el-dropdown-link" type="text">{{
                      snsOptLeft.snsInfo[snsOptLeft.snsOptInd].label
                    }}</el-button>
                    <el-dropdown-menu slot="dropdown" :-to-body="false">
                      <el-dropdown-item :command="item.id + ''" :icon="item.icon" v-for="item in snsOptLeft.snsInfo"
                        :key="item.id">{{ item.label }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </div>
                <div class="size_info">
                  <span>{{ this.streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].encInfo }}</span>
                </div>
              </div>
              <div class="right_controls_container">
                <div class="draw_option">
                  <el-tooltip effect="dark" content="缩小" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-remove" @click="zoomOut(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, 0.1)" @mousedown.native="startZoomOut(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, true)" @mouseup.native="stopZoomOut(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, true)" @mouseleave.native="stopZoomOut(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, true)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" content="放大" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-circle-plus" @click="zoomIn(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, 0.1)" @mousedown.native="startZoomIn(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, true)" @mouseup.native="stopZoomIn(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, true)" @mouseleave.native="stopZoomIn(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, true)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="ezoom" >
                  <el-slider class="ezoom_slider" v-model="ezoom[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].ezoom_ratio" :min="1" :max="max_ezoom_ratio_0" :step="0.1" @mousedown.native="onEZoomMousedown(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)" @input="onEZoom(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)" v-show="ezoomEnable"></el-slider>
                </div>
                <div class="draw_option" >
                  <el-tooltip effect="dark" :content="is3ASyncOn ? '关闭3A同步' : '开启3A同步'" placement="top-end" :enterable="false">
                    <el-button
                      size="mini"
                      :class="{'draw_button_enable':is3ASyncOn, 'draw_button_disable': !is3ASyncOn}"
                      icon="el-icon-refresh" @click="change3ASync()"
                      v-show="preview_sns_mode === 0"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="!isTalking ? '开始对讲' : '停止对讲'" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" :icon="isTalking?'iconfont icon-mic-off':'iconfont icon-mic-on'" @click="onStartTalk()"
                      v-show="aenc_enable && sns0CurDisplayMediaType != 'MJPEG' && snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id === 0"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="!isMuted ? '静音' : '播音'" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" :icon="isMuted?'iconfont icon-silence':'iconfont icon-sound'" @click="onMute()"
                      v-show="aenc_enable && sns0CurDisplayMediaType != 'MJPEG' && snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id === 0"></el-button>
                  </el-tooltip>
                </div>
                <div class="trigger_flash">
                  <el-tooltip effect="dark" :content="'爆闪'" placement="top-end">
                    <el-button size="mini" :class="{ 'draw_button_enable': true, 'draw_button_disable': false }"
                     icon ="el-icon-thumb" @click="doTriggerFlash(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)" v-show="streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].trigger_attr.trigger_enable"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="'抓拍'" placement="top-end">
                    <el-button size="mini" :class="{ 'draw_button_enable': true, 'draw_button_disable': false }"
                      icon="el-icon-camera-solid" @click="onSnapshot(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)"
                      v-show="snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id === 0"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="'录入人像信息'" placement="top-end" :enterable="false">
                    <el-button size="mini" :class="{ 'draw_button_enable': true, 'draw_button_disable': false }"
                      icon="el-icon-user-solid" @click="recordFaceinfo(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)"
                      v-show="isSearchImg && detect_mode == 'facehuman'"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="!streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].isRecStart ? '开启录像' : '停止录像'" placement="top-end"
                    :enterable="false">
                    <el-button size="mini" :disabled="!streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].recEnable"
                      class="draw_button_enable" :icon="streamOptions[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].recIcon"
                      @click="onRecording(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)"></el-button>
                  </el-tooltip>
                </div>
                <div class="full_option">
                  <el-tooltip effect="dark" :content="isFullScreen ? '退出全屏' : '进入全屏'" placement="top-end"
                    :enterable="false">
                    <el-button size="mini" class="full_button" :icon="screenIcon" @click="doScreen(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)"></el-button>
                  </el-tooltip>
                </div>
              </div>
            </el-footer>
        ...
        <div class="preview_seperator_line" v-show="preview_sns_mode === 2"></div>
          ...
          <el-container id="video_container_1" class="preview_container">
            ...
            <el-footer class="video_controls_container" height="30px">
              <div class="left_controls_container">
                ...
                <div class="size_info">
                  <span calss="dynamic_font">{{ this.streamOptions[1].encInfo }}</span>
                </div>
              </div>
              <div class="right_controls_container">
                <div class="draw_option">
                  <el-tooltip effect="dark" content="缩小" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-remove" @click="zoomOut(1, 0.1)" @mousedown.native="startZoomOut(1, true)" @mouseup.native="stopZoomOut(1, true)" @mouseleave.native="stopZoomOut(1, true)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" content="放大" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-circle-plus" @click="zoomIn(1, 0.1)" @mousedown.native="startZoomIn(1, true)" @mouseup.native="stopZoomIn(1, true)" @mouseleave.native="stopZoomIn(1, true)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="ezoom">
                  <el-slider class="ezoom_slider" v-model="ezoom[1].ezoom_ratio" :min="1" :max="max_ezoom_ratio_1" :step="0.1" @mousedown.native="onEZoomMousedown(1)" @input="onEZoom(1)" v-show="ezoomEnable"></el-slider>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="'抓拍'" placement="top-end">
                    <el-button size="mini" :class="{ 'draw_button_enable': true, 'draw_button_disable': false }"
                ...
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="!streamOptions[1].isRecStart ? '开启录像' : '停止录像'" placement="top-end" :enterable="false">
                    <el-button size="mini" :disabled="!streamOptions[1].recEnable"
                      class="draw_button_enable" :icon="streamOptions[1].recIcon"
...
export default {
  props: ["mode_option", "preview_sns_mode", "max_ezoom_ratio_0", "max_ezoom_ratio_1"],
  data() {
    return {
      image_count: 16,
      detect_mode: 'hvcfp',
      ...
      ezoomMouseDown: [false, false, false],
      ezoom: [
        {
          ezoom_ratio: 1.0,
          center_offset_x: 0,
      ...
      ezoomTimer: [
        {
          timerZoomIn: undefined,
          timerZoomOut: undefined
        },
        {
          timerZoomIn: undefined,
          timerZoomOut: undefined
      ...
      ezoom_throttle_interval: 30,
      ezoom_longpress_interval: 150,
      snsOptLeft: {
        snsOptInd: 0,
        snsInfo: [
          {
  ...
  methods: {
    ...
    async startPlay() {
      let CurLeftSnsId = this.snsOptLeft.snsInfo[this.snsOptLeft.snsOptInd].id
    ...
    play(snsId) {
      ...
      try {
        ...
        if (Wfs.isSupported()) {
          ...
          if (this.aenc_enable && snsId == 0 && displayInfo.mediaType != 'MJPEG') {
            audioInfo = {
          ...
          if (snsId == 0) {
            this.sns0CurDisplayMediaType = displayInfo.mediaType
          }
    ...
    stop(snsId) {
      ...
      this.stopZoomIn(snsId, false)
      this.stopZoomOut(snsId, false)
    ...
    changeSizeOption0(command) {
      this.changeSizeOption(this.snsOptLeft.snsInfo[this.snsOptLeft.snsOptInd].id, Number(command))
    },
    ...
    async changeStream0(command, replay = true, changeData = true) {
      await this.changeStream(this.snsOptLeft.snsInfo[this.snsOptLeft.snsOptInd].id, Number(command), replay, changeData)
    },
    ...
    async changeSnsOptLeft(command) {
      let curSnsId = this.snsOptLeft.snsInfo[this.snsOptLeft.snsOptInd].id
      this.stop(curSnsId)
      this.stopEncInfoQuery(curSnsId)
      let streamInd = this.streamOptions[curSnsId].streamInd
      let snsId = Number(command)
      console.log('changesnsOptLeft sns id: ' + snsId + " stream id: " + streamInd)
      for (var i = 0; i < this.snsOptLeft.snsInfo.length; i++) {
        if (snsId == this.snsOptLeft.snsInfo[i].id) {
          this.snsOptLeft.snsOptInd = i
    ...
    initSensorOptInd(snsNum) {
      ...
      if (ind == undefined || snsNum < 3) {
        this.snsOptLeft.snsOptInd = 0
      }
      else {
        ind = parseInt(ind)
        if (ind > 1) {
    ...
    async getInfo(changeSNSVideo, snsId) {
      try {
        ...
        if (res.meta.status === 200) {
          ...
          this.snsOptLeft.snsInfo.splice(1)
          for (var k = 0; k < this.sns_num; k++) {
            ...
            if (k >= 2) {
              if (k % 2 == 0) {
                ...
                this.snsOptLeft.snsInfo.push(_item)
              }
            }
            ...
            if (k < this.ezoom.length) { // update ezoom item
              this.ezoom.splice(k, 1, _item)
            } else { // add ezoom item
              this.ezoom.splice(k, 0, _item)
            }
          }
    ...
    recResultCallback0(result) {
      ...
      if (result) {
        ...
        this.streamOptions[0].recIcon = 'iconfont icon-record-stop'
      }
      else {
        this.$message.error('Cam0开启录像失败')
      }
    },
    recResultCallback1(result) {
      ...
      if (result) {
        ...
        this.streamOptions[1].recIcon = 'iconfont icon-record-stop'
      }
      else {
        this.$message.error('Cam1开启录像失败')
      }
    },
    recResultCallback2(result) {
      this.streamOptions[2].recEnable = true
      if (result) {
        this.streamOptions[2].isRecStart = true
        this.streamOptions[2].recIcon = 'iconfont icon-record-stop'
      }
      else {
        this.$message.error('Cam2开启录像失败')
      }
    },
    ...
    recStateCallback2(state) {
      this.streamOptions[2].recEnable = state
    },
    ...
    async onSnapshot(snsId) {
      ...
      try {
        ...
        this.$message.error('发送抓拍命令失败')
      }
    },
    async doLastZoom(snsId, delayTime) {
        // make sure last slider value handled
        await this.sleep(delayTime)
        this.doEZoom(snsId)
        console.log('[doLastZoom] ezoom ratio: ', this.ezoom[snsId].ezoom_ratio)
    },
    onEZoomMousedown(snsId) {
      this.ezoomMouseDown[snsId] = true
      window.onmouseup = () => {
        this.ezoomMouseDown[snsId] = false
        window.onmouseup = null
        // make sure last slider value handled
        this.doLastZoom(snsId, this.ezoom_throttle_interval + 10)
      };
    },
    async doEZoom(snsId) {
      let stream_id = this.streamOptions[snsId].videoInfo[this.streamOptions[snsId].streamInd].id;
      try {
        ...
        // this.$message.error('发送电子放大命令失败')
      }
    },
    onEZoom: throttle(async function(snsId) {
      if (!this.ezoomMouseDown[snsId]) {
        return
      }
      this.doEZoom(snsId)
      // console.log('[onEZoom] ezoom ratio: ', this.ezoom[snsId].ezoom_ratio)
    }, 30,/* equal to 'this.ezoom_throttle_interval' */ { leading: true, trailing: false }),
    async zoomIn(snsId, step) {
      if (snsId == 1) {
        if (this.ezoom[snsId].ezoom_ratio + step > this.max_ezoom_ratio_1) {
          this.ezoom[snsId].ezoom_ratio = this.max_ezoom_ratio_1
        } else {
          this.ezoom[snsId].ezoom_ratio = parseFloat((this.ezoom[snsId].ezoom_ratio + step).toFixed(1))
        }
      } else {
        if (this.ezoom[snsId].ezoom_ratio + step > this.max_ezoom_ratio_0) {
          this.ezoom[snsId].ezoom_ratio = this.max_ezoom_ratio_0
        } else {
          this.ezoom[snsId].ezoom_ratio = parseFloat((this.ezoom[snsId].ezoom_ratio + step).toFixed(1))
        }
      }
    ...
    async zoomOut(snsId, step) {
      if (this.ezoom[snsId].ezoom_ratio - step < 1) {
          this.ezoom[snsId].ezoom_ratio = 1
      } else {
        this.ezoom[snsId].ezoom_ratio = parseFloat((this.ezoom[snsId].ezoom_ratio - step).toFixed(1))
      }
      this.ezoomMouseDown[snsId] = true
    },
    async startZoomIn(snsId) {
      this.ezoomTimer[snsId].timerZoomIn = setInterval(() => {
        this.zoomIn(snsId, 0.1)
      }, this.ezoom_longpress_interval)
    },
    stopZoomIn(snsId, isDoLastZoom) {
      if (this.ezoomTimer[snsId].timerZoomIn !== undefined) {
        clearInterval(this.ezoomTimer[snsId].timerZoomIn)
        this.ezoomTimer[snsId].timerZoomIn = undefined
        if (isDoLastZoom) {
          // make sure last slider value handled
          this.doLastZoom(snsId, this.ezoom_throttle_interval + 10)
        }
    ...
    async startZoomOut(snsId) {
      this.ezoomTimer[snsId].timerZoomOut = setInterval(() => {
        this.zoomOut(snsId, 0.1)
      }, this.ezoom_longpress_interval)
    },
    stopZoomOut(snsId, isDoLastZoom) {
      if (this.ezoomTimer[snsId].timerZoomOut !== undefined) {
        clearInterval(this.ezoomTimer[snsId].timerZoomOut)
        this.ezoomTimer[snsId].timerZoomOut = undefined
        if (isDoLastZoom) {
          // make sure last slider value handled
          this.doLastZoom(snsId, this.ezoom_throttle_interval + 10)
        }
      ...
      if (this.streamOptions[snsId].wfsObj) {
        ...
        if (!is_rec_start) {
          ...
          this.streamOptions[snsId].wfsObj.start_recording(rec_threshold_web * 1024 * 1024,
            (0 === snsId) ? this.recResultCallback0 : (1 === snsId) ? this.recResultCallback1 : this.recResultCallback2,
            (0 === snsId) ? this.recStateCallback0 : (1 === snsId) ? this.recStateCallback1 : this.recStateCallback2)
        } else {
          this.streamOptions[snsId].wfsObj.stop_recording()
          this.streamOptions[snsId].isRecStart = false
          this.streamOptions[snsId].recIcon = 'iconfont icon-record-start'
      ...
      try {
        ...
        if (res.meta.status === 200) {
          ...
          this.streamOptions[snsId].encInfo = 'CAM' + snsId + '\xa0\xa0'
            + '编码格式: ' + mediaTypeStr + '\xa0\xa0'
            + '分辨率: ' + res.data.assist_res + '\xa0\xa0'
    ...
    async change3ASync () {
      console.log('++ enable 3A Sync:' + !this.is3ASyncOn)
      const { data: res } = await this.$http.post('preview/3a_sync', { sync3a: !this.is3ASyncOn })
      console.log('post enable 3A Sync return: ', res)
      if (res.meta.status !== 200) return this.$message.error('设置3A Sync状态失败')
      this.is3ASyncOn = !this.is3ASyncOn
      console.log('-- enable 3A Sync:' + this.is3ASyncOn)
    },
    doScreen(snsId) {
            ...
            reader.onload = (evt) => {
              if (evt.target.readyState === FileReader.DONE) {
                ...
                for (var i = 0; i < eventObj.events.length; i++) {
                  ...
                  } else if (eventObj.events[i].type === 5) {
                    this.eventList.unshift({ type: '声音检测', date: eventObj.events[i].date, sensor: eventObj.events[i].db + 'dB' })
    ...
    async resetVideoStream(snsId) {
      this.stopSensorPreview(snsId)
      console.log('sleep 200ms and reset video stream')
