<template>
  <div>
    <el-form ref="formCameraRef" :rules="formCameraRules" :model="formCamera" label-width="100px" size="mini">
      ...
      <el-row>
        <el-form-item label="工作模式:" v-show="formCamera.camera_attr.switch_work_mode_enable" class="inline">
          <el-select v-model="formCamera.camera_attr.sns_work_mode">
            <el-option v-for="item in loadedSnsModeItems" :key="item.label" :label="item.label" :value="item.value"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="Ratio:" v-show="formCamera.camera_attr.hdr_ratio_enable && formCamera.camera_attr.sns_work_mode == 2" class="inline">
      ...
      <el-form-item label="相机分辨率:" v-show="resolution_options.length > 0">
         <el-select v-model="formCamera.camera_attr.sns_resolution">
          <el-option v-for="item in resolution_options" :key="item.label" :label="item.label" :value="item.value">
         </el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button :disabled="!data_valid" type="primary" @click="onSubmit">修改</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  computed: {
    loadedSnsModeItems() {
      return this.sns_mode_options.slice(0, this.sns_mode_item_num)
    },
  },
  methods: {
    onSubmit() {
    ...
    async getInfo() {
      try {
        ...
        if (res.meta.status === 200) {
          ...
          this.formCamera.camera_attr.sns_resolution = res.data.camera_attr.sns_resolution
          ...
          this.formCamera.camera_attr.switch_capture_enable = res.data.camera_attr.switch_capture_enable
          ...
          if (res.data.camera_attr.lf_hdr_support) {
            ...
            this.sns_mode_item_num = 2
          }

          var fps = []
          for (let index = 0; index < this.formCamera.framerate_opts.length; index++) {
            fps.push({ label: '' + this.formCamera.framerate_opts[index], value: this.formCamera.framerate_opts[index] })
          }
          this.framerate_options = fps

          var resolution_opts = []
          for (let index = 0; index < this.formCamera.resolution_opts.length; index++) {
            resolution_opts.push({ label: '' + this.formCamera.resolution_opts[index], value: index })
          }
          this.resolution_options = resolution_opts
        } else {
