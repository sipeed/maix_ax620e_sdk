...
AXLiveServerMediaSession* AXLiveServerMediaSession::createNewVideo(UsageEnvironment& env, bool reuseFirstSource, AX_PAYLOAD_TYPE_E ePt, AX_U32 nMaxFrmSize) {
    return new AXLiveServerMediaSession(env, reuseFirstSource, ePt, nMaxFrmSize, 0, 0, 0);
}

AXLiveServerMediaSession* AXLiveServerMediaSession::createNewAudio(UsageEnvironment& env, bool reuseFirstSource, AX_PAYLOAD_TYPE_E ePt, AX_U32 nMaxFrmSize, AX_U32 nSampleRate, AX_U8 nChnCnt, AX_S32 nAOT) {
    return new AXLiveServerMediaSession(env, reuseFirstSource, ePt, nMaxFrmSize, nSampleRate, nChnCnt, nAOT);
}
AXLiveServerMediaSession::AXLiveServerMediaSession(UsageEnvironment& env, bool reuseFirstSource, AX_PAYLOAD_TYPE_E ePt, AX_U32 nMaxFrmSize, AX_U32 nSampleRate, AX_U8 nChnCnt, AX_S32 nAOT)
    ...
    m_pIFrameBuf = new AX_U8[nMaxFrmSize];
...
char const* AXLiveServerMediaSession::getAuxSDPLine(RTPSink* rtpSink, FramedSource* inputSource) {
    ...
    if (fDummySink != NULL && fDoneFlag == 0) {
        setDoneFlag();
        fDummySink = NULL;
...
FramedSource* AXLiveServerMediaSession::createNewStreamSource(unsigned clientSessionID, unsigned& estBitRate) {
    ...
    if (PT_H264 == m_ePt || PT_H265 == m_ePt) {
        estBitRate = 8192;
    } else {
        estBitRate = 48;
    }
    ...
    m_pSource = source;
    if (m_pIFrameBuf && m_nIFrameSize > 0) {
        m_pSource->AddFrameBuff(m_nIFrameChn, m_pIFrameBuf, m_nIFrameSize, m_nIFramePts, AX_TRUE);
    }
...
void AXLiveServerMediaSession::SendNalu(AX_U8 nChn, const AX_U8* pBuf, AX_U32 nLen, AX_U64 nPts /*=0*/, AX_BOOL bIFrame /*=AX_FALSE*/) {
    pthread_spin_lock(&m_tLock);
    if (bIFrame && nLen < m_nMaxFrmBuffSize && nLen > 0) {
        memcpy(m_pIFrameBuf, pBuf, nLen);
        m_nIFrameSize = nLen;
        m_nIFramePts = nPts;
        m_nIFrameChn = nChn;
    }
