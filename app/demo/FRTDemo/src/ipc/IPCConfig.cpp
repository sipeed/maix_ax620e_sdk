...
AX_BOOL CIPCConfig::InitOsdConfig(AX_VOID) {
    ...
    if (E_APP_SCENARIO_DEFAULT == nScenario
        || E_APP_SCENARIO_TRIPLE_SENSOR == nScenario
        || E_APP_SCENARIO_TRIPLE_SENSOR_SWITCH_SINGLE_PIPE == nScenario) {
        /* sensor1 primary*/
        vecOsdCfg.clear();
        // TIME
        tOsdConfig.bEnable = AX_TRUE;
    ...
    if (E_APP_SCENARIO_TRIPLE_SENSOR == nScenario) {
        /* sensor1 primary*/
        vecOsdCfg.clear();
        // TIME
        tOsdConfig.bEnable = AX_TRUE;
        ...
        tOsdConfig.eType = OSD_TYPE_RECT;
        vecOsdCfg.push_back(tOsdConfig);
        CWebOptionHelper::GetInstance()->SetOsdConfig(2, 2, 0, vecOsdCfg);
    }
...
AX_VOID CIPCConfig::OsdVideoChnIndex2IvpsGrp(AX_VOID) {
    ...
    if (E_APP_SCENARIO_TRIPLE_SENSOR == nScenario) {
        /* sensor1*/
        pair<AX_U8, AX_U8> tVenc = make_pair(2, 0);
        pair<AX_U8, AX_U8> tIvps = make_pair(2, 0);
        pairVideo2Ivps[tVenc] = tIvps;
    }
