<template>
  <div class="total_container">
    <el-container class="preview_outside_container" >
      <el-container :class="mode_option == 'ai' ? 'preview_main_container_ai' : 'preview_main_container_normal'">
              ...
              <div class="right_controls_container">
                <div class="draw_option">
                  <el-tooltip effect="dark" content="缩小" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-remove" @click="ZoomOut(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, 0.1)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" content="放大" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-circle-plus" @click="ZoomIn(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id, 0.1)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="ezoom" >
                  <el-slider class="ezoom_slider" v-model="ezoom[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].ezoom_ratio" :min="1" :max="ezoom[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].max_ratio" :step="ezoom[snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id].step" @mousedown.native="onEZoomMousedown(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)" @input="onEZoom(snsOptLeft.snsInfo[snsOptLeft.snsOptInd].id)" v-show="ezoomEnable"></el-slider>
                </div>
                <div class="draw_option">
          ...
          <el-container id="video_container_1" class="preview_container">
            ...
            <el-footer class="video_controls_container" height="30px">
              ...
              <div class="right_controls_container">
                <div class="draw_option">
                  <el-tooltip effect="dark" content="缩小" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-remove" @click="ZoomOut(1, 0.1)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" content="放大" placement="top-end" :enterable="false">
                    <el-button size="mini" class="draw_button_enable" icon="el-icon-circle-plus" @click="ZoomIn(1, 0.1)" v-show="ezoomEnable"></el-button>
                  </el-tooltip>
                </div>
                <div class="ezoom">
                  <el-slider class="ezoom_slider" v-model="ezoom[1].ezoom_ratio" :min="1" :max="ezoom[1].max_ratio" :step="ezoom[1].step" @mousedown.native="onEZoomMousedown(1)" @input="onEZoom(1)" v-show="ezoomEnable"></el-slider>
                </div>
                <div class="draw_option">
                  <el-tooltip effect="dark" :content="'抓拍'" placement="top-end">
                    <el-button size="mini" :class="{ 'draw_button_enable': true, 'draw_button_disable': false }"
...
export default {
  props: ["mode_option", "preview_sns_mode"],
  data() {
    return {
      image_count: 16,
      detect_mode: 'hvcfp',
  ...
  methods: {
    ...
    play(snsId) {
      ...
      try {
        ...
        if (!tokenStr) {
          console.log('play--, snsid: ' + snsId + ',token is empty')
          return
        }

        let sizeOptionInd = this.streamOptions[snsId].sizeOptionInd
        let streamInd = this.streamOptions[snsId].streamInd
        let streamId = this.streamOptions[snsId].videoInfo[streamInd].id
        let mediaFPS = this.streamOptions[snsId].videoInfo[streamInd].mediaFPS
        this.changeSizeOption(snsId, sizeOptionInd)
        if (!this.streamOptions[snsId].videoInfo[streamInd].enable) {
          console.log('play--, snsid: ' + snsId + ', streamInd' + streamInd + ', is disabled!')
          return
        }
        ...
        if (Wfs.isSupported()) {
          ...
          this.streamOptions[snsId].wfsObj.attachMedia(media, 'ch' + snsId, displayInfo.mediaType, 'ws/preview' + '?token=' + tokenStr + '&&sns_id=' + snsId + "&&chn_id=" + streamId, displayInfo.extra, audioInfo)
          ...
          for (var k = 0; k < this.sns_num; k++) {
            ...
            for (var i = 0; i < res.data.info.stream_list[k].length; i++) {
              ...
              var _item = {
                ...
                enable: res.data.info.stream_list_enable[k][i]
              }
            ...
            var _item = {
              ...
              max_ratio: res.data.info.ezoom[k].max_ratio,
              step: res.data.info.ezoom[k].step
            }
	...
	this.ezoom[snsId].ezoom_ratio = this.ezoom[snsId].max_ratio
      } else {
    ...
    startPushImage() {
      ...
      if (!tokenStr)
          return
      var uri = 'ws://' + window.location.host + '/ws/pushimage?token=' + tokenStr
      var protocol = 'binary'
      this.wsPushImage = new WebSocket(uri, protocol)

      this.wsPushImage.onopen = () => {
        console.log('PushImage connected')
        this.createPushImageTimer()
      }
      this.wsPushImage.onclose = (err) => {
        console.log('PushImage disconnected', err)
        if (!this.isForceStop) {
          setTimeout(() => {
            this.startPushImage()
          }, 2000)
      ...
      this.wsPushImage.onmessage = (event) => {
        if (typeof (event.data) === 'string') {
          console.log('PushImage got a message: ' + event.data)
        } else {
          // console.log('got a PushImage data, data-type:' + typeof (event.data) )
          try {
            ...
            console.log('PushImage image catch except: ' + err)
          }
        }
      }
      this.wsPushImage.onerror = (err) => {
        console.log('PushImage occure error', err)
      }
    },
    startSnapshot() {
      this.isForceStop = true
      ...
      if (!tokenStr)
          return
      var uri = 'ws://' + window.location.host + '/ws/snapshot?token=' + tokenStr
      var protocol = 'binary'
      this.wsSnapshot= new WebSocket(uri, protocol)

      this.wsSnapshot.onopen = () => {
        console.log('Snapshot connected')
        this.createSnapshotTimer()
      }
      this.wsSnapshot.onclose = (err) => {
        console.log('Snapshot disconnected', err)
        if (!this.isForceStop) {
          setTimeout(() => {
            this.startSnapshot()
          }, 2000)
      ...
      this.wsSnapshot.onmessage = (event) => {
        if (typeof (event.data) === 'string') {
          console.log('Snapshot got a message: ' + event.data)
        } else {
          // console.log('got a snapshot data, data-type:' + typeof (event.data) )
          try {
            const blob = new Blob([event.data], { type: 'image/jpeg' })
            const fileName = 'snapshot_' + this.getCurrentTime() + '.jpg'
            saveAs(blob, fileName)
          } catch (err) {
            console.log('Snapshot catch except: ' + err)
          }
        }
      }
      this.wsSnapshot.onerror = (err) => {
        console.log('Snapshot occure error', err)
      }
    },
    startAudioComm() {
      this.wsTalkBufList = []
      ...
      this.wsTalk.onmessage = (event) => {
        if (typeof (event.data) === 'string') {
          console.log('got a talk message: ' + event.data)
        }
      }
    ...
    stopPushImage() {
      this.clearPushImageTimer()
      if (this.wsPushImage) {
        this.wsPushImage.close()
      }
    },
    createPushImageTimer() {
      this.clearPushImageTimer()
      this.timerPushImage = setInterval(() => {
        if (this.wsPushImage && this.wsPushImage.readyState === WebSocket.OPEN) {
          this.wsPushImage.send("keep-alive")
        }
      }, 3000)
    },
    clearPushImageTimer() {
      if (this.timerPushImage) {
        clearInterval(this.timerPushImage)
        this.timerPushImage = undefined
      }
    },
    stopSnapshot() {
      this.clearSnapshotTimer()
      if (this.wsSnapshot) {
        this.wsSnapshot.close()
      }
    },
    createSnapshotTimer() {
      this.clearSnapshotTimer()
      this.timerSnapshot = setInterval(() => {
        if (this.wsSnapshot && this.wsSnapshot.readyState === WebSocket.OPEN) {
          this.wsSnapshot.send("keep-alive")
        }
      }, 3000)
    },
    clearSnapshotTimer() {
      if (this.timerSnapshot) {
        clearInterval(this.timerSnapshot)
        this.timerSnapshot = undefined
      }
    },
    startRecvEvents() {
