<template>
  <div>
    <el-form ref="formImageRef" :rules="formImageRules" :model="formImage" label-width="100px" size="mini">
      ...
      <el-form-item label="数字防抖:" v-show="formImage.dis_attr.dis_support">
        <el-switch v-model="formImage.dis_attr.dis_enable"></el-switch>
      </el-form-item>
      <el-form-item>
    ...
    async getInfo() {
      try {
        ...
        if (res.meta.status === 200) {
          ...
          //distor_ratio 负值时为字符串，这里用*1转换成数值
          this.formImage.ldc_attr.distor_ratio = res.data.ldc_attr.distor_ratio * 1

          this.formImage.dis_attr.dis_support = res.data.dis_attr.dis_support
          this.formImage.dis_attr.dis_enable = res.data.dis_attr.dis_enable
        } else {
