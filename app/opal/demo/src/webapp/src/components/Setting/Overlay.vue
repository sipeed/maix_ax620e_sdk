  ...
  <div>
    <el-container>
        ...
        <el-aside class="preview_container" width="600px">
          <div class="videoContainer" id="videoContainer">
            ...
            <div id="osdBound" class="osdBound">
              ...
              <div id="targetOSDPrivacy" class="targetOSDPrivacy" v-show="this.formOverlay.privacy_attr.enable">
                <canvas id="osdPrivacy" class="osdPrivacy" width="600px" height="400px" v-show="this.formOverlay.privacy_attr.enable">
                </canvas>
              </div>
              <div id="targetOSDPrivacyPt0" class="targetOSDPrivacyPt0" v-show="this.formOverlay.privacy_attr.enable">
              </div>
              <div id="targetOSDPrivacyPt1" class="targetOSDPrivacyPt1" v-show="this.formOverlay.privacy_attr.enable">
              </div>
              <div id="targetOSDPrivacyPt2" class="targetOSDPrivacyPt2" v-show="this.formOverlay.privacy_attr.enable && this.formOverlay.privacy_attr.type != 0">
              </div>
              <div id="targetOSDPrivacyPt3" class="targetOSDPrivacyPt3" v-show="this.formOverlay.privacy_attr.enable && this.formOverlay.privacy_attr.type != 0">
              </div>
            </div>
          </div>
        </el-aside>
          ...
          <div>
            ...
            <el-tabs tab-position="left" style="height: 200px;">
              <el-tab-pane label="时间标题">
                <el-form ref="formOverlayRef" :rules="formOverlayRules" :model="formOverlay.overlay_attr[chn_cur]" label-width="100px">
                  ...
                  <el-form-item label="启用反色：">
                    <el-switch v-model="formOverlay.overlay_attr[chn_cur].time.inverse_enable" :disabled="!formOverlay.overlay_attr[chn_cur].time.enable" ></el-switch>
                  </el-form-item>
                  <el-form-item label="反色颜色：">
                    <el-select v-model="formOverlay.overlay_attr[chn_cur].time.color_inv" size="mini" :disabled="!formOverlay.overlay_attr[chn_cur].time.enable">
                      <el-option v-for="item in colorOptions" :key="item.label" :label="item.label" :value="item.value"> </el-option>
                    </el-select>
                  </el-form-item>
                </el-form>
              ...
              <el-tab-pane label="位置标题">
                <el-form ref="formOverlayRef" :rules="formOverlayRules" :model="formOverlay.overlay_attr[chn_cur]" label-width="100px">
                  ...
                  <el-form-item label="内容：">
                    <el-input v-model="formOverlay.overlay_attr[chn_cur].location.text" size="mini" maxlength="128" :disabled="!formOverlay.overlay_attr[chn_cur].location.enable" @input="onLocationTextChanged"></el-input>
                  </el-form-item>
                  <el-form-item label="启用反色：">
                    <el-switch v-model="formOverlay.overlay_attr[chn_cur].location.inverse_enable" :disabled="!formOverlay.overlay_attr[chn_cur].location.enable"></el-switch>
                  </el-form-item>
                  <el-form-item label="反色颜色：">
                    <el-select v-model="formOverlay.overlay_attr[chn_cur].location.color_inv" size="mini" :disabled="!formOverlay.overlay_attr[chn_cur].location.enable">
                      <el-option v-for="item in colorOptions" :key="item.label" :label="item.label" :value="item.value"> </el-option>
                    </el-select>
                  </el-form-item>
                </el-form>
              ...
              <el-tab-pane label="通道标题">
                <el-form ref="formOverlayRef" :rules="formOverlayRules" :model="formOverlay.overlay_attr[chn_cur]" label-width="100px">
                  ...
                  <el-form-item label="内容：">
                    <el-input v-model="formOverlay.overlay_attr[chn_cur].channel.text" size="mini" maxlength="128" :disabled="!formOverlay.overlay_attr[chn_cur].channel.enable" @input="onChannelTextChanged"></el-input>
                  </el-form-item>
                  <el-form-item label="启用反色：">
                    <el-switch v-model="formOverlay.overlay_attr[chn_cur].channel.inverse_enable" :disabled="!formOverlay.overlay_attr[chn_cur].channel.enable"></el-switch>
                  </el-form-item>
                  <el-form-item label="反色颜色：">
                    <el-select v-model="formOverlay.overlay_attr[chn_cur].channel.color_inv" size="mini" :disabled="!formOverlay.overlay_attr[chn_cur].channel.enable">
                      <el-option v-for="item in colorOptions" :key="item.label" :label="item.label" :value="item.value"> </el-option>
                    </el-select>
                  </el-form-item>
                </el-form>
              ...
              <el-tab-pane label="隐私遮挡">
                <el-form ref="formOverlayRef" :rules="formOverlayRules" :model="formOverlay.overlay_attr[chn_cur]" label-width="100px">
                  <el-form-item label="显示：">
                    <el-switch v-model="formOverlay.privacy_attr.enable"></el-switch>
                  </el-form-item>
                  <el-form-item label="类型：">
                    <el-select v-model="formOverlay.privacy_attr.type" size="mini" :disabled="!formOverlay.privacy_attr.enable" @change="onChangePrvType">
                      <el-option v-for="item in privacyTypeOptions" :key="item.label" :label="item.label" :value="item.value"> </el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="线宽：" v-show="formOverlay.privacy_attr.type != OSD_PRIVACY_TYPE_MOSA">
                    <el-select v-model="formOverlay.privacy_attr.linewidth" size="mini" :disabled="!formOverlay.privacy_attr.enable">
                      <el-option v-for="item in Array.from({ length: 5 }, (item, index) => index + 1)" :key="item" :label="'' + item" :value="item"> </el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="大小：" v-show="formOverlay.privacy_attr.type == OSD_PRIVACY_TYPE_MOSA">
                    <el-select v-model="formOverlay.privacy_attr.linewidth" size="mini" :disabled="!formOverlay.privacy_attr.enable">
                      <el-option v-for="item in Array.from({ length: 5 }, (item, index) => index + 1)" :key="item" :label="'' + item" :value="item"> </el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="颜色：" v-show="formOverlay.privacy_attr.type != OSD_PRIVACY_TYPE_MOSA">
                    <el-select v-model="formOverlay.privacy_attr.color" size="mini" :disabled="!formOverlay.privacy_attr.enable">
                      <el-option v-for="item in colorOptions" :key="item.label" :label="item.label" :value="item.value"> </el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="填充：" v-show="formOverlay.privacy_attr.type != OSD_PRIVACY_TYPE_MOSA">
                    <el-switch v-model="formOverlay.privacy_attr.solid" :disabled="!formOverlay.privacy_attr.enable"></el-switch>
                  </el-form-item>
                </el-form>
              </el-tab-pane>
            </el-tabs>
      ...
      <el-footer class="overlayFooter">
        ...
        <!-- <el-button :disabled="!data_valid" type="primary" @click="onFresh" size="mini">刷新</el-button> -->
      </el-footer>
    </el-container>
  </div>
</template>
  ...
  methods: {
    ...
    play() {
      ...
      try {
        ...
        if (!this.streamOptions[this.formOverlay.src_id][this.chn_cur].enable) {
          console.log('play--, snsid: ' + this.formOverlay.src_id + ', streamInd' + this.chn_cur + ', is disabled!')
          return
        }
        ...
        if (Wfs.isSupported()) {
          ...
          let sns_id = this.formOverlay.src_id
          let chn_id = this.streamOptions[sns_id][this.chn_cur].id

          this.wfsObj.attachMedia(media, 'ch' + chn_id, mediaType, 'ws/preview' + '?token=' + tokenStr + '&&sns_id=' + sns_id + "&&chn_id=" + chn_id, extra)
    ...
    async getInfo() {
      ...
      try {
        ...
        if (res.meta.status === 200) {
          ...
          this.formOverlay.privacy_attr = res.data.privacy_attr
    ...
    async getVideoInfo() {
      try {
        ...
        if (res.meta.status === 200) {
          this.data_valid = true
          this.sns_num = res.data.info.sns_num >= 2 ? 2 : 1;

          for (var k = 0; k < this.sns_num; k++) {
            this.streamOptions[k] = []
            for (var i = 0; i < res.data.info.stream_list[k].length; i++) {
              ...
              this.streamOptions[k].push(_item)
            }
          }
    ...
    drawOSDLocation() {
      ...
      try {
        ...
        context.fillStyle = 'red'
        var txt = this.formOverlay.overlay_attr[this.chn_cur].location.text
        var txt_w = context.measureText(txt).width
        if (this.formOverlay.overlay_attr[this.chn_cur].location.align == this.OSD_ALIGN_TYPE_RIGHT_BOTTOM || this.formOverlay.overlay_attr[this.chn_cur].location.align == this.OSD_ALIGN_TYPE_RIGHT_TOP) {
          context.fillText(txt, w - txt_w, h - 2)
        } else {
          context.fillText(txt, 1, h - 2)
        }
        console.log("location str width: " + txt_w)
      } catch (error) {
    ...
    drawOSDChannel() {
      ...
      try {
        ...
        var txt_w = context.measureText(txt).width
        if (this.formOverlay.overlay_attr[this.chn_cur].channel.align == this.OSD_ALIGN_TYPE_RIGHT_BOTTOM || this.formOverlay.overlay_attr[this.chn_cur].channel.align == this.OSD_ALIGN_TYPE_RIGHT_TOP) {
          context.fillText(txt, w - txt_w, h - 2)
        } else {
          context.fillText(txt, 1, h - 2)
        }
    ...
    drawOSDPrivacy() {
      try {
        var bounding = document.getElementById("osdBound").getBoundingClientRect()
        var x0 = this.formOverlay.privacy_attr.points[0][0] / this.formOverlay.privacy_attr.sourceResolution[0] * bounding.width
        var y0 = this.formOverlay.privacy_attr.points[0][1] / this.formOverlay.privacy_attr.sourceResolution[1] * bounding.height
        var x1 = this.formOverlay.privacy_attr.points[1][0] / this.formOverlay.privacy_attr.sourceResolution[0] * bounding.width
        var y1 = this.formOverlay.privacy_attr.points[1][1] / this.formOverlay.privacy_attr.sourceResolution[1] * bounding.height
        var x2 = this.formOverlay.privacy_attr.points[2][0] / this.formOverlay.privacy_attr.sourceResolution[0] * bounding.width
        var y2 = this.formOverlay.privacy_attr.points[2][1] / this.formOverlay.privacy_attr.sourceResolution[1] * bounding.height
        var x3 = this.formOverlay.privacy_attr.points[3][0] / this.formOverlay.privacy_attr.sourceResolution[0] * bounding.width
        var y3 = this.formOverlay.privacy_attr.points[3][1] / this.formOverlay.privacy_attr.sourceResolution[1] * bounding.height
        ...
        var x, y, w, h
        if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_LINE) {
          var xs = [x0, x1]
          var ys = [y0, y1]
        ...
        } else if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_POLY) {
          var xs = [x0, x1, x2, x3]
          var ys = [y0, y1, y2, y3]
          x = Math.min.apply(null, xs)
          y = Math.min.apply(null, ys)
        ...
        if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_LINE) {
          ...
          this.formOverlay.privacy_attr.points[0][0] = (x / bounding.width * this.formOverlay.privacy_attr.sourceResolution[0])
          this.formOverlay.privacy_attr.points[0][1] = (y / bounding.height * this.formOverlay.privacy_attr.sourceResolution[1])
          eleTarget = document.getElementById("targetOSDPrivacyPt1")
          eleTarget.style.left = (x + w - 2) + 'px'
          eleTarget.style.top = (y - 2) + 'px'
          this.formOverlay.privacy_attr.points[1][0] = ((x + w) / bounding.width * this.formOverlay.privacy_attr.sourceResolution[0])
          this.formOverlay.privacy_attr.points[1][1] = (y / bounding.height * this.formOverlay.privacy_attr.sourceResolution[1])
          eleTarget = document.getElementById("targetOSDPrivacyPt2")
          eleTarget.style.left = (x + w - 2) + 'px'
          eleTarget.style.top = (y + h - 2) + 'px'
          this.formOverlay.privacy_attr.points[2][0] = ((x + w) / bounding.width * this.formOverlay.privacy_attr.sourceResolution[0])
          this.formOverlay.privacy_attr.points[2][1] = ((y + h) / bounding.height * this.formOverlay.privacy_attr.sourceResolution[1])
          eleTarget = document.getElementById("targetOSDPrivacyPt3")
          eleTarget.style.left = (x - 2) + 'px'
          eleTarget.style.top = (y + h - 2) + 'px'
          this.formOverlay.privacy_attr.points[3][0] = (x / bounding.width * this.formOverlay.privacy_attr.sourceResolution[0])
          this.formOverlay.privacy_attr.points[3][1] = ((y + h) / bounding.height * this.formOverlay.privacy_attr.sourceResolution[1])
        }
    ...
    onOsdDragEnd(target, index) {
      ...
      if (index === 0) {
        ...
        x = parseInt(parseInt(target.style.left) / bounding.width * this.formOverlay.privacy_attr.sourceResolution[0])
        y = parseInt(parseInt(target.style.top) / bounding.height * this.formOverlay.privacy_attr.sourceResolution[1])
        w = parseInt(parseInt(target.style.width) / bounding.width * this.formOverlay.privacy_attr.sourceResolution[0])
        h = parseInt(parseInt(target.style.height) / bounding.height * this.formOverlay.privacy_attr.sourceResolution[1])
        var x0 = this.formOverlay.privacy_attr.points[0][0]
        var y0 = this.formOverlay.privacy_attr.points[0][1]
        var x1 = this.formOverlay.privacy_attr.points[1][0]
        var y1 = this.formOverlay.privacy_attr.points[1][1]
        var x2 = this.formOverlay.privacy_attr.points[2][0]
        var y2 = this.formOverlay.privacy_attr.points[2][1]
        var x3 = this.formOverlay.privacy_attr.points[3][0]
        var y3 = this.formOverlay.privacy_attr.points[3][1]
        ...
        if (index === 4) {
          if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_LINE) {
            ...
            this.formOverlay.privacy_attr.points[0] = [(x0 + xd) < 0 ? 0 : (x0 + xd), (y0 + yd) < 0 ? 0 : (y0 + yd)]
            this.formOverlay.privacy_attr.points[1] = [(x1 + xd) < 0 ? 0 : (x1 + xd), (y1 + yd) < 0 ? 0 : (y1 + yd)]
          } else if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_POLY) {
            var xs0 = [x0, x1, x2, x3]
            var ys0 = [y0, y1, y2, y3]
            var xd = x - Math.min.apply(null, xs0)
            ...
            // console.log("privacy points old:" + this.formOverlay.privacy_attr.points)
            this.formOverlay.privacy_attr.points[0] = [(x0 + xd) < 0 ? 0 : (x0 + xd), (y0 + yd) < 0 ? 0 : (y0 + yd)]
            this.formOverlay.privacy_attr.points[1] = [(x1 + xd) < 0 ? 0 : (x1 + xd), (y1 + yd) < 0 ? 0 : (y1 + yd)]
            this.formOverlay.privacy_attr.points[2] = [(x2 + xd) < 0 ? 0 : (x2 + xd), (y2 + yd) < 0 ? 0 : (y2 + yd)]
            this.formOverlay.privacy_attr.points[3] = [(x3 + xd) < 0 ? 0 : (x3 + xd), (y3 + yd) < 0 ? 0 : (y3 + yd)]
          } else {
            this.formOverlay.privacy_attr.points = [[x, y], [x + w, y], [x + w, y + h], [x, y + h]]
          }
          //console.log("privacy points new:" + this.formOverlay.privacy_attr.points)
        }
        else if (index === 5) {
          // pt0
          this.formOverlay.privacy_attr.points[0] = [x + 2, y + 2]
          if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_RECT || this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_MOSA) {
            if (x0 < x2) {
              this.formOverlay.privacy_attr.points[1] = [x2, y0]
              this.formOverlay.privacy_attr.points[3] = [x0, y2]
            } else {
              this.formOverlay.privacy_attr.points[1] = [x0, y2]
              this.formOverlay.privacy_attr.points[3] = [x2, y0]
            }
          }
        }
        else if (index === 6) {
          // pt1
          this.formOverlay.privacy_attr.points[1] = [x + 2, y + 2]
          if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_RECT || this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_MOSA) {
            if (x1 > x3) {
              this.formOverlay.privacy_attr.points[0] = [x3, y1]
              this.formOverlay.privacy_attr.points[2] = [x1, y3]
            } else {
              this.formOverlay.privacy_attr.points[0] = [x1, y3]
              this.formOverlay.privacy_attr.points[2] = [x3, y1]
            }
          }
        }
        else if (index === 7) {
          // pt2
          this.formOverlay.privacy_attr.points[2] = [x + 2, y + 2]
          if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_RECT || this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_MOSA) {
            if (x0 < x2) {
              this.formOverlay.privacy_attr.points[1] = [x2, y0]
              this.formOverlay.privacy_attr.points[3] = [x0, y2]
          ...
          // pt3
          this.formOverlay.privacy_attr.points[3] = [x + 2, y + 2]
          if (this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_RECT || this.formOverlay.privacy_attr.type === this.OSD_PRIVACY_TYPE_MOSA) {
            if (x1 > x3) {
              this.formOverlay.privacy_attr.points[0] = [x3, y1]
              this.formOverlay.privacy_attr.points[2] = [x1, y3]
