...
AX_VOID *GenARGB(wchar_t *pStr, AX_U16 *pArgbBuffer, AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_S16 sX, AX_S16 sY,
    ...
    if (u32OSDWidth > OSD_PIXEL_MAX_WIDTH || u32OSDWidth <= 0) {
        LOG_M_E(LOG_TAG, "osd width is out of range.(%d > %d)", u32OSDWidth, OSD_PIXEL_MAX_WIDTH);
        return NULL;
    }

    if (u32OSDHeight > OSD_PIXEL_MAX_HEIGHT || u32OSDHeight <= 0) {
        LOG_M_E(LOG_TAG, "osd height is out of range. (%d > %d)", u32OSDHeight, OSD_PIXEL_MAX_HEIGHT);
        return NULL;
    }
    ...
    if (0 != s32Error) {
        LOG_M_E(LOG_TAG, "FT Set Size error");
        return NULL;
    }
    FT_Bitmap *bitmap;
    AX_S32 sBearingY = 0;
    ...
    switch (enAlign) {
        ...
        default:
            LOG_M_E(LOG_TAG, "unsupported align type");
            return NULL;
    }
...
AX_VOID *GenBitmap(wchar_t *pStr, AX_U8 *pBitmapBuffer, AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_S16 sX, AX_S16 sY,
                                AX_U16 uFontSize, OSD_ALIGN_TYPE_E enAlign) {
    AX_S16 x = sX;
    AX_S16 y = sY;
...
AX_BOOL InitOSDHandler(const AX_CHAR *fontFilePath) {
    ...
    if (0 != s32Error) {
        LOG_M_E(LOG_TAG, "FT_Init_FreeType error");
        return AX_FALSE;
    }

    s32Error = FT_New_Face(m_fontLibrary, fontFilePath, 0, &m_fontFace);
    if (FT_Err_Unknown_File_Format == s32Error) {
        LOG_M_E(LOG_TAG, "FT_New_Face unknown file format");
        return AX_FALSE;
    } else if (0 != s32Error) {
        LOG_M_E(LOG_TAG, "cannot open font file! errorNo:%d", s32Error);
        return AX_FALSE;
    }
...
FT_Bitmap *FTGetGlpyhBitMap(AX_U16 u16CharCode) {
    ...
    if (0 != s32Error) {
        LOG_M_E(LOG_TAG, "FT_Load_Glyph failed");
        return NULL;
    }
...
AX_S32 CalcStrSize(wchar_t *pTextStr, AX_U16 uFontSize, AX_U32 *u32OSDWidth, AX_U32 *u32OSDHeight) {
